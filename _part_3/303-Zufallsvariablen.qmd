...
ist dabei die Möglichkeit im Rahmen der probabilistischen Modellformulierung, die
Verteilungen von Zufallsvariablen mithilfe von Wahrscheinlichkeitsmassefunktion 
und Wahrscheinlichkeitsdichtefunktionen festzulegen. Weiterhin impliziert die 
Konstruktion von Zufallsvariablen als Abbildungen zufälliger Ergebnisse mit der
Untersuchung der Verteilungen der so transformierten zufälligen Ergebnisse das 
Kernthema frequentistischer Inferenz. 

## Definition {#sec-zufallsvariablen-definition}
...
von $S$ gegeben durch (vgl. @def-bildmenge-wertebereich-urbildmenge-urbild)
\begin{equation}
\xi^{-1}(S) := \{\omega \in \Omega|\xi(\omega) \in S\}.
\end{equation}
Wenn nun $\xi^{-1}(S) \in \mathcal{A}$ für alle $S \in \mathcal{S}$ gilt,
dann nennt man die Abbildung $\xi$ *messbar*. Nehmen wir also an 
...
![Konstruktion von Zufallsvariable und Verteilung.](./_figures/303-zufallsvariable){#fig-zufallsvariable fig-align="center" width=80%}

:::{#def-zufallsvariable}
## Zufallsvariable
Es sei $(\Omega, \mathcal{A}, \mathbb{P})$ ein Wahrscheinlichkeitsraum und
$(\mathcal{X},\mathcal{S})$ ein *Messraum*. Dann ist eine *Zufallsvariable*
definiert als eine Abbildung $\xi:\Omega \to \mathcal{X}$ mit der *Messbarkeitseigenschaft*
\begin{equation}
...
auf $\xi(\omega)$ abgebildet. Wir definieren dementsprechend die Begriffe des
...
sei eine Zufallsvariable. Dann heißt $\mathcal{X}$ der *Ergebnisraum der Zufallsvariable $\xi$* 
und ein $\xi(\omega) \in \mathcal{X}$ heißt eine *Realisierung der Zufallvariable $\xi$*.
:::
...
* $\mathbb{P} : \mathcal{A} \to [0,1]$ mit $\mathbb{P}(\{(r,b)\}) = \frac{1}{36}$ für alle $(r,b) \in \Omega$.

gegeben ist. Die Auswertung der Summe der beiden Würfelaugenzahlen wird dann 
sinnvollerweise durch die Abbildung 
\begin{equation}
\xi : \Omega \to \mathcal{X}, (r,b) \mapsto \xi((r,b)) := r + b
\end{equation}
beschrieben, wobei offenbar $\mathcal{X} := \{2,3,...,12\}$ gelten muss. Der
Ergebnisraum der Zufallsvariable ist also wiederum endlich und
$\mathcal{S} := \mathcal{P}(\mathcal{X})$ ist eine sinnvolle $\sigma$-Algebra 
auf $\mathcal{X}$. Mithilfe der $\sigma$-Additivität von $\mathbb{P}$ können wir 
...
von $\xi$ nachweisen, wie in @tbl-wuerfel-summe gezeigt.
...
: Wahrscheinlichkeitsverteilung der Augensumme beim Wurf zweier Würfel {#tbl-wuerfel-summe}

\normalsize

Die Wahrscheinlichkeiten der Elementarereignisse in $\mathcal{S}$ wiederum erlauben
das Berechnen beliebiger Ereigniswahrscheinlichkeiten hinsichtlich der Würfelaugenzahlsumme 
(vgl. @sec-wahrscheinlichkeitsfunktionen). Insgesamt haben wir basierend auf 
$(\Omega, \mathcal{A}, \mathbb{P})$ und $\xi$ also ein weiteres Wahrscheinlichkeitsraummodell 
...
die Bestimmung des Wertes eins psychologischen Fragebogens (BDI-II) an zufällig ausgewählten
...
die Gesamtheit aller infrage kommender Proband:innen darstellen und die Auswahl
...
![Zufallsvariable als Modell eines Messvorgangs.](./_figures/303-zufallsvariable-messen){#fig-zufallsvariable-messen fig-align="center" width=80%}

### Notation {-}

Die Konventionen zur Notation der mit Zufallsvariablen assoziierten Wahrscheinlichkeiten
und Verteilungen sind etwas gewöhnungsbedürftig, sodass wir sie in folgender
...
Der Verzicht auf die explizite Bezeichnung von Funktionen eindeutig bestimmter
Zufallsvariablen, wie er in @def-notation-für-zufallsvariablen bezüglich $\mathbb{P}_\xi$
und $\mathbb{P}$ angerissen wird, ist ein grundlegendes Phänomen moderner, anwendungsorientierter 
Wahrscheinlichkeitstheorie. Speziell werden in den meisten Fällen Funktionen von Zufallsvariablen
mithilfe ihrer Argumente identifiziert. So sollte der verständigen Leser:in anhand
von "$\mathbb{P}\left(\xi \in S\right)$" klar sein, dass es bei $\mathbb{P}$ um das
Bildmaß der Zufallsvariable $\xi$ geht, bei "$\mathbb{P}\left(\{\omega \in \Omega|\xi(\omega) \in S\}\right)$"
dagegen um das Wahrscheinlichkeitsmaß auf dem Messraum $(\Omega, \mathcal{A})$.
Die Identifikation von Funktionen anhand ihrer Argumente, die insbesondere auch
bei den in folgenden Abschnitten diskutierten *Wahrscheinlichkeitsmassefunktionen*,
*Wahrscheinlichkeitsdichtefunktionen* und *kumulativen Verteilungsfunktionen* zum 
Tragen kommt, bezeichnet man in der informatischen Anwendung auch als *Multimethode (multiple dispatch)*.  

Zum Abschluss dieses Abschnittes halten wir in @thm-arithmetik-reeller-zufallsvariablen
fest, dass arithmetische Operationen mit reellwertigen Zufallsvariablen wiederum
auf reellwertige Zufallsvariablen führen.
...
$\xi_1 : \Omega \to \mathbb{R}$ und $\xi_2: \Omega \to  \mathbb{R}$ seien 
reellwertige Zufallsvariablen und $c \in \mathbb{R}$ sei eine Konstante. Weiterhin seien
\begin{align}
\begin{split}
\xi_1 + c          & : \Omega \to \mathbb{R}, \omega \mapsto (\xi_1 + c)(\omega)\,             := \xi_1(\omega) + c              \mbox{ für } c \in \mathbb{R}     \\
c\xi_1             & : \Omega \to \mathbb{R}, \omega \mapsto (c\xi_1)(\omega) \quad\,\,\,      := c\xi_1(\omega)                 \mbox{ für } c \in \mathbb{R}     \\
\xi_1 + \xi_2      & : \Omega \to \mathbb{R}, \omega \mapsto (\xi_1 + \xi_2)(\omega)           := \xi_1(\omega) + \xi_2(\omega)                                      \\
\xi_1\xi_2         & : \Omega \to \mathbb{R}, \omega \mapsto (\xi_1\xi_2)(\omega)\quad\,\,     := \xi_1(\omega)\xi_2(\omega)                                        \\
\end{split}
\end{align}
...
Multiplikation zweier reellwertigen Zufallsvariablen, respektive. Dann sind auch $\xi_1 + c$, $c\xi_1$,
$\xi_1 + \xi_2$ und $\xi_1\xi_2$ reellwertige Zufallsvariablen.
:::
...
dass die Addition einer zufälligen Größe zu einer konstanten Größe, die Multiplikation 
einer zufälligen Größe mit einer Konstante, die Addition zweier zufälliger Größen 
und die Multiplikation zweier zufälliger Größen immer wieder zufällige Größen 
mit ihren dann eigenen Verteilungen ergeben.
...
Eine Zufallsvariable $\xi$ heißt *diskret*, wenn ihr Ergebnisraum $\mathcal{X}$
...
Eine entsprechende Funktion $p$ heißt *Wahrscheinlichkeitsmassefunktion (WMF)* von $\xi$.
:::
...
an diesem Begriff orientieren wir uns hier. Ohne Beweis halten wir fest, dass 
jede Funktion $p : \mathcal{X} \to \mathbb{R}_{\ge 0}$, die die Normiertheitseigenschaft
$\sum_{x \in \mathcal{X}} p(x) = 1$ besitzt als WMF einer Zufallsvariable 
interpretiert werden kann. 

### Beispiele {-}

Wir wollen mit den *Bernoulli-Zufallsvariablen*, den *Binomial-Zufallsvariablen*
und den *diskret-gleichverteilten Zufallsvariablen* drei erste Beispiele für 
...
\end{equation}
Dann sagen wir, dass $\xi$ einer *Bernoulli-Verteilung mit Parameter
$\mu \in [0,1]$* unterliegt und nennen $\xi$ eine *Bernoulli-Zufallsvariable*.
...
für $x \in \{0,1\}$ Sinn ergibt und nicht etwa für $x \in \{\mbox{H}, \mbox{T}\}$.
Bildet man aber die möglichen Ergebnisse eines Münzwurfes auf $\{0,1\}$ ab, also
definiert etwa $0 := \mbox{H}$ und $1 := \mbox{T}$, so kann eine Bernoulli-Zufallsvariable 
...
von 
\begin{equation}
\mathbb{P}(\xi = 1) = p(1) = \mu^1 (1 -\mu)^{1-1} = \mu.
\end{equation}
...
library(latex2exp)
pdf(
file      =  "./_figures/303-bernoulliverteilung-wmf.pdf",
width     = 7,
height    = 3)
par(                                                                   
mfcol     = c(1,length(mu)),                                            
...
![WMFen von Bernoulli-Zufallsvariablen.](./_figures//303-bernoulliverteilung-wmf){#fig-bernoulliverteilung-wmf fig-align="center" width=100%}
...
\end{equation}
Dann sagen wir, dass $\xi$ einer *Binomialverteilung mit Parametern $n \in \mathbb{N}$
und $\mu \in [0,1]$* unterliegt und nennen $\xi$ eine *Binomial-Zufallsvariable*.
Wir kürzen dies mit $\xi \sim \mbox{Bin}(n,\mu)$ ab. Die WMF einer
...
genutzt werden kann. Insbesondere gilt also $\mbox{Bin}(x;1,\mu) = \mbox{Bern}(x;\mu)$.
...
für $(n,\mu) := (5,0.1),  (n,\mu) := (10,0.5)$ und $(n,\mu) := (15,0.7)$ in
...
# Visualisierung
pdf(
file       = file.path("./_figures/303-binomialverteilung-wmf.pdf"),
width      = 7,
height     = 3)
par(                                                                    
mfcol       = c(1,length(mu)),                                          
...
for (i in 1:length(n)){
  ...
  xlim         = c(-.1,n[i]+.1),                                             
  ylim         = c(0,.6))                                                   
  title(sprintf("Bin(x;%g,%g)", n[i], mu[i]))                               
}
dev.off()
```

![WMFen von Binomial-Zufallsvariablen.](./_figures/303-binomialverteilung-wmf){#fig-binomialverteilung-wmf fig-align="center" width=100%}
...
\end{equation}
Dann sagen wir, dass $\xi$ einer *diskreten Gleichverteilung* unterliegt
und nennen $\xi$ eine *diskret-gleichverteilte Zufallsvariable*. Wir kürzen
...
für $\mathcal{X} := \{\{0,1\}$, $\mathcal{X} := \{-3,-2,-1,0,1\}\}$ und 
$\mathcal{X} := \{-4,-3,-2,-1,0,1,2,3,4\}$ in @fig-gleichverteilung-wmf. 
Man beachte, dass @def-diskret-gleichverteilte-zufallsvariable formal die 
Sequentialität der Elemente von $\mathcal{X}$ nicht erfordert, man
...
# Visualisierungsparameter
pdf(
file        = file.path("./_figures/303-gleichverteilung-wmf.pdf"),
width       = 7,
height      = 3)
par(                                                                    
mfcol       = c(1,length(mu)),                                           
...
![WMFen von diskret-gleichverteilten Zufallsvariablen.](./_figures/303-gleichverteilung-wmf){#fig-gleichverteilung-wmf fig-align="center" width=100%}
...
Mit der Gamma-Zufallsvariable, der Beta-Zufallsvariable und der gleichverteilten
...
Eine Zufallsvariable $\xi$ heißt *kontinuierlich*, wenn $\mathbb{R}$ der 
...
(2) $\mathbb{P}(\xi \in [a,b]) = \int_a^b p(x)\,dx$ für alle $a,b\in\mathbb{R}$ mit $a \le b$.

Eine entsprechende Funktion $p$ heißt *Wahrscheinlichkeitsdichtefunktion (WDF)* von $\xi$.
:::


Im Englischen nennt man WDFen *probability density functions (PDFs)*. Ohne Beweis 
halten wir fest, dass jede Funktion $p : \mathbb{R} \to \mathbb{R}_{\ge 0}$, 
für deren uneigentliches Integral $\int_{-\infty}^{\infty} p(x)\,dx = 1$ gilt,
...
sondern Wahrscheinlichkeitsdichten dar, Wahrscheinlichkeiten werden aus WDFen anhand von 
@def-kontinuierliche-zufallsvariable-und-wahrscheinlichkeitsdichtefunktion (2)
durch Integration berechnet. Wie im physikalischen Sinn ergibt sich die einem reellen 
Intervall zugeordnete Wahrscheinlichkeitsmasse also erst durch "Multiplikation"
mit dem entprechenden "Intervallvolumen". Man denke hierzu auch an die Approximation 
des bestimmten Integrals $\int_a^b p(x)\,dx$ durch einen Riemannschen Summenterm 
...
\begin{equation}
\mathbb{P}(\xi = a) = \int_a^a p(x) \,dx = 0.
\end{equation}
...
die grafische Darstellung der WDF einer Zufallsvariable und stellt sich eine 
...
die Definition einer kontinuierlichen Zufallsvariable mithilfe einer WDF nun 
die wichtigste Zufallsvariable der probabilistischen Modellbildung einführen.

:::{#def-zufallsvariablen-normalverteilte-zufallsvariable}
...
$(\mu,\sigma^2) := (0,1), (\mu,\sigma^2) := (-2.5, 10)$ und $(\mu,\sigma^2) := (3, 0.5)$ 
in @fig-normalverteilung-wdf. Man macht sich an dieser Abbildung grafisch klar, 
dass die WDFen von normalverteilten Zufallsvariablen immer genau einen Wert höchster 
Wahrscheinlichkeitsdichte besitzen - und zwar an der Stelle des Parameters $\mu \in \mathbb{R}$. 
...
bei $x = \mu$, also $x-\mu = 0$ annimmt. Weiterhin macht man sich grafisch klar, 
dass der Parameter $\sigma^2>0$ die Breite der WDF einer normalverteilten 
Zufallsvariable enkodiert.
...
library(latex2exp)
pdf(
file    = file.path("./_figures/303-normalverteilung-wdf.pdf"),
width   = 7,
height  = 3)
par(                                                                   
mfcol      = c(1,length(mu)),                                           
...
![WDFen von normalverteilten Zufallsvariablen.](./_figures//303-normalverteilung-wdf){#fig-normalverteilung-wdf fig-align="center" width=100%}

### Weitere Beispiele {-}

Wir wollen mit den *Gamma-Zufallsvariablen*, den *Beta-Zufallsvariablen*
und den *gleichverteilten Zufallsvariablen* drei weitere Beispiele für die 
Definition von Verteilungen mithilfe von WDFen betrachten.
...
die WDFen von Gamma-Zufallsvariablen für  $(\alpha,\beta) := (1,1)$, 
$(\alpha,\beta) := (2,2)$ und $(\alpha,\beta) := (5,1)$ in @fig-gammaverteilung-wdf.
...
# Visualisierung
pdf(
file        = "_figures/303-gammaverteilung-wdf.pdf",
width       = 7,
height      = 3)
par(                                                                    
...
dev.off()
```
![WDFen von Gamma-Zufallsvariablen.](./_figures/303-gammaverteilung-wdf){#fig-gammaverteilung-wdf fig-align="center" width=100%}
...
\end{equation}
wobei $\Gamma$ die Gammafunktion bezeichne. Dann sagen wir, dass $\xi$ einer
*Beta-Verteilung* mit Parametern $\alpha >0$ und $\beta>0$ unterliegt,
und nennen $\xi$ eine *Beta-Zufallsvariable*. Wir kürzen dies mit
$\xi \sim \mbox{Beta}(\alpha,\beta)$ ab. Die WDF einer Beta-Zufallsvariable
bezeichnen wir mit
\begin{equation}
...
# Visualisierung
pdf(
file   = "_figures/303-betaverteilung-wdf.pdf",
width  = 7,
height = 3)
par(                                                                    
mfcol      = c(1,length(alpha)),                                               
...
![WDFen von Beta-Zufallsvariablen.](./_figures/303-betaverteilung-wdf){#fig-betaverteilung-wdf fig-align="center" width=100%}
...
kürzen dies mit $\xi \sim U(a,b)$ ab. Die WDF einer gleichverteilten Zufallsvariablen
bezeichnen wir mit
\begin{equation}
U(x;a,b) := \frac{1}{b - a}.
\end{equation}
...
Wir visualisieren die WDFen von gleichverteilten Zufallsvariablen für $(a,b) := (0,1)$, 
$(a,b) := (-3,1)$ und $(a,b) := (-4,4)$ in @fig-gleichverteilung-wdf.
...
# Visualisierung
pdf(
file      = "_figures/303-gleichverteilung-wdf.pdf",
width     = 7,
height    = 3)
...
![WDFen von gleichverteilten-Zufallsvariablen.](./_figures/303-gleichverteilung-wdf){#fig-gleichverteilung-wdf fig-align="center" width=100%}

## Kumulative Verteilungsfunktionen {#sec-kumulative-verteilungsfunktion}

```{r, eval = FALSE, echo = FALSE}
pdf(                                                                    
file      = "_figures/303-bernoulliverteilung-kvf.pdf",                
width     = 6.50,                                                                
height    = 4.75)  
mu        = c(.1,.5,.7)                                                            
par(                                                                  
...
for (i in 1:length(mu)){
  ...
  x         = -1:2
  kvf       = stepfun(x, c(0, pbinom(x, 1, mu[i])), right = FALSE) 
  plot(
  kvf,
  lwd       = 1.0,
  col       = "Blue",
...
```{r, eval = FALSE, echo = FALSE}
pdf(                                                                    
file      = "_figures/303-binomialverteilung-kvf.pdf",
width     = 6.50,                                                                 
height    = 4.75)      
n         = c(5,10,15)                                                          
mu        = c(.1,.5,.7)                                                                                                                               
...
for (i in 1:length(n)){                                                          
  ...
  xlim         = c(-.1,n[i]+.1),                                            
  ylim         = c(0,.6))                                                  
  title(sprintf("WMF von Bin(x;%g,%g)", n[i], mu[i]))    
  kvf         =  stepfun(x, c(0, pbinom(x, n[i], mu[i])), right = FALSE)
  plot(
  kvf,
  lwd         = 1,
  col         = "Blue",
...
# Visualisierung
pdf(                                                                    
file      = "_figures/303-normalverteilung-kvf.pdf",        
width     = 6.50,                                                      
height    = 4.75)
par(                                                                   
mfcol     = c(2,length(mu)),                                          
...
# Visualisierung
pdf(
file        = "_figures/303-normalverteilung-ikvf.pdf",                   
width       = 6.50,                                                                  
height      = 4.75)                                                                                   
par(                                                                   
mfcol       = c(2,length(mu)),                                          
...
nützlich, da sie sowohl für diskrete als auch kontinuierliche Zufallsvariablen 
...
\end{equation}
:::
Man beachte, dass $P(x)$ für jedes $x \in \mathbb{R}$ definiert ist, 
auch wenn für ein gegebenes $x\in \mathbb{R}$ gilt, dass $x \notin \mathcal{X}$.
Mithilfe von KVFen können zum Beispiel *Überschreitungswahrscheinlichkeiten* und 
*Intervallwahrscheinlichkeiten* von Zufallsvariablen direkt ausgewertet werden. 
Dies ist der Inhalt folgender Theoreme.

:::{#thm-ueberschreitungswahrscheinlichkeit}
...
erfüllt, kann als eine KVF einer Zufallsvariablen interpretiert werden. Für einen
...
(1) $P$ ist *monoton steigend*, das heißt, wenn $x_1 < x_2$, dann gilt $P(x_1)\le P(x_2)$.
(2) $\lim_{x \to -\infty} P(x) = 0$ und $\lim_{x \to \infty} P(x) = 1$.
(3) $P$ ist *rechtsseitig stetig*, d.h., $P(x) = \lim_{y \to x, y > x} P(y)$ für alle $x \in \mathbb{R}$
:::
...
für den Ergebniswert $x$ der Zufallsvariable $\xi$ der Wahrscheinlichkeit 
$\mathbb{P}(\xi \le x)$ entspricht, also der Wahrscheinlichkeit dafür ist, dass 
...
intuitiv: Wenn $a < b$ und $\mathbb{P}(a < \xi < b) = 0$ ist, dann ist die KVF 
...
diskreten Zufallsvariablen mit Ergebnisraum $\mathbb{N}_0$ durch
\begin{equation}
...
![WMFen und KVFen von Bernoulli-Zufallsvariablen.](./_figures/303-bernoulliverteilung-kvf){#fig-bernoulliverteilung-kvf fig-align="center" width=100%}  

![WMFen und KVFen von Binomial-Zufallsvariablen.](./_figures/303-binomialverteilung-kvf){#fig-binomialverteilung-kvf fig-align="center" width=100%}  
...
$\xi$ sei eine kontinuierliche Zufallsvariable mit WDF $p$ und KVF $P$. Dann gelten
\begin{equation}
...
$x \in \mathbb{R}$ gilt, die KVF von $\xi$ keine Sprünge hat, $P$ also stetig ist.
Mit den Definitionen von WDF und KVF, folgt, dass $P$ die Form einer Stammfunktion
von $p$ hat. Dass $p$ die Ableitung von $P$ ist, folgt dann direkt aus dem ersten
Hauptsatz der Differenzial- und Integralrechnung (@thm-erster-hauptsatz-der-differenzial-und-integralrechnung).
:::
...
Ableitung der KVF ist. Im Kontext des *Theorems von Radon-Nikodym* wird diese Einsicht
...
Als Beispiel betrachten wir die KVF einer normalverteilten Zufallsvariablen. Es sei
...
Zufallsvariablen nur numerisch, nicht aber analytisch berechnet werden. 
Wir visualisieren ausgewählte WDFen und KVFen von normalverteilten Zufallsvariablen
in @fig-normalverteilung-kvf. 

![WDFen und KVFen von normalverteilten Zufallsvariablen.](./_figures/303-normalverteilung-kvf){#fig-normalverteilung-kvf fig-align="center" width=100%}  

### Inverse kumulative Verteilungsfunktion {-}

Wir beschließen diesen Abschnitt mit dem Begriff der *inversen kumulativen Verteilungsfunktion*,
...
$\xi \sim N(0,1)$ gelten beispielsweise, dass
\begin{equation}
P(1.645) = 0.950 
\Leftrightarrow
P^{-1}(0.950) = 1.645,
...
![KVFen und inverse KVFen von normalverteilten Zufallsvariablen.](./_figures/303-normalverteilung-ikvf){#fig-normalverteilung-ikvf fig-align="center" width=100%}  

## Zufallsergebnisse und Zufallsvariablen

Mit den in @sec-wahrscheinlichkeitsraeume diskutierten *Ergebnissen $\omega$* und den
in diesem Kapitel diskutierten Werten von *Zufallsvariablen $\xi$* haben wir nun zwei
Konzepte kennengelernt, die die Unsicherheit über einen zumeist numerischen Wert
...
Ergebnisses oder einer Zufallsvariablen vorstellen. Tatsächlich gibt es auch 
keine standardisierte Antwort auf die Frage, ob man einen Zufallsvorgang lediglich 
mit einem Wahrscheinlichkeitsraum oder mit dem zugrundeliegenden Wahrscheinlichkeitsraum
einer Zufallsvariablen und dem durch beide induzierten Wahrscheinlichkeitsraum 
modellieren sollte. In der Anwendung wird meist der Begriff der Zufallsvariablen
bevorzugt und entsprechende WMFen oder WDFen angegeben, ohne dass ein zugrundeliegender
Wahrscheinlichkeitsraum oder die Abbildungsform der Zufallsvariablen spezifiziert würde.
...
Zufallsvariable (vgl. @def-zufallsvariablen-normalverteilte-zufallsvariable) für $\xi$ der 
...
die Identitätsfunktion ist, ergänzt werden. Konkret könnte man für den zugrundeliegenden
...
\end{equation}
wählen. Da $\xi$ dann an dem im Rahmen des zugrundeliegenden Wahrscheinlichkeitsraums
zufällig realisiertem Ergebnis $\omega$ nichts ändert und dieses lediglich in $x$
umbenannt wird, entspricht die Verteilung von $\xi$ hier direkt dem Wahrscheinlichkeitsmaß 
des zugrundeliegenden Wahrscheinlichkeitsraums. 
...
enthält, über dies hinausgehend aber die Möglichkeit eröffnet, durch von der 
Identitätsabbildung verschiedene Zufallsvariablen die Transformation von 
Wahrscheinlichkeitsmaßen auf unterschiedlichen Messräumen zu formalisieren und zu studieren.

## Literaturhinweise

Die Genese des Begriffs der Zufallsvariablen ist eng mit der Entwicklung der
Wahrscheinlichkeitstheorie in den letzten drei Jahrhunderten verflochten, sodass 
keine für den Begriff entscheidene Publikation angegeben werden kann. Die mathematische 
Entwicklung des Begriffs der Normalverteilung durch Abraham De Moivre 
...
1. Erläutern Sie die intuitive Bedeutung von $\mathbb{P}(\xi = x)$.
...
1. Schreiben Sie die Überschreitungswahrscheinlichkeit $\mathbb{P}(\xi > x)$ einer Zufallsvariable mithilfe ihrer KVF.
1. Schreiben Sie die Intervallwahrscheinlichkeit $\mathbb{P}(\xi \in ]x_1,x_2]$ einer Zufallsvariable mithilfe ihrer KVF.
1. Schreiben Sie den Wert $P(x)$ der KVF einer kontinuierlichen Zufallsvariable mithilfe ihrer WDF $p$.
1. Schreiben Sie den Wert $p(x)$ der WDF einer kontinuierlichen Zufallsvariable mithilfe ihrer KVF $P$.
1. Geben Sie die Definition des Begriffs der inversen Verteilungsfunktion wieder.

## Lösungen {.unnumbered .unlisted}

1. Siehe @def-zufallsvariable.
1. Die Gleichung besagt, dass der von der Verteilung $\mathbb{P}_\xi$ der Zufallsvariable
$\xi$ zugeordnete Wahrscheinlichkeitswert für das Ereignis $\xi = x$ in $\mathcal{X}$ 
definitionsgemäß gleich dem Wert des von dem Wahrscheinlichkeitsmaß $\mathbb{P}$ 
dem Urbild des Ereignisses $\xi = x$, also der Menge $\{\xi = x\} = \{\omega \in \Omega|\xi(\omega) = x\}$
in $\mathcal{A}$, ist. Siehe @def-notation-für-zufallsvariablen.
1. Intuitiv ist $\mathbb{P}(\xi = x)$ die Wahrscheinlichkeit dafür, dass die Zufallsvariable $\xi$ den Wert $x$ annimmt. 
1. Siehe @def-diskrete-zufallsvariable-und-wahrscheinlichkeitsmassefunktion.
1. Siehe @def-kontinuierliche-zufallsvariable-und-wahrscheinlichkeitsdichtefunktion.
1. Siehe @def-zufallsvariablen-normalverteilte-zufallsvariable.
1. Siehe @def-kumulative-verteilungsfunktion.
1. Siehe @thm-ueberschreitungswahrscheinlichkeit.
1. Siehe @thm-intervallwahrscheinlichkeiten.
1. Siehe @thm-kumulative-verteilungsfunktionen-von-kontinuierlichen-zufallsvariablen. Es gilt 
\begin{equation}
P(x) = \int_{-\infty}^x p(s)\,ds.
\end{equation}
1. Siehe @thm-kumulative-verteilungsfunktionen-von-kontinuierlichen-zufallsvariablen. Es gilt 
\begin{equation}
p(x) = \frac{d}{dx}P(x) = P'(x).
\end{equation}
1. Siehe @def-inverse-kumulative-verteilungsfunktion.
