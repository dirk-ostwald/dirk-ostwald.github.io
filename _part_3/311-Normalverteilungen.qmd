# Normalverteilungen {#sec-normalverteilungen}
\normalsize
...
## Konstruktion multivariater Normalverteilungen {#sec-konstruktion-multivariater-normalverteilungen}
...
Begriff der normalverteilten Zufallsvariable. Wir wiederholen die entsprechende
Definition aus @sec-zufallsvariablen hier, um die Terminologie für die multivariaten
Normalverteilungen einzuführen.
...
mit $\xi \sim N(\mu,\sigma^2)$ ab. Die WDF einer normalverteilten Zufallsvariablen
bezeichnen wir mit
\begin{equation}
...
library(latex2exp)
pdf(
file        = file.path("./_figures/311-univariate-normal-wdf.pdf"),
width       = 9,
height      = 3)
par(
mfcol      = c(1,3),
...
![Wahrscheinlichkeitsdichtefunktionen univariater Normalverteilungen.](./_figures/311-univariate-normal-wdf){#fig-univariate-normal-wdf fig-align="center" width=100%}

Visuell entspricht der Parameter $\mu$ einer normalverteilten Zufallsvariablen
...
Erwartungswert und die Varianz einer normalverteilten Zufallsvariablen bekanntlich
\begin{equation}
...
:::{#exm-konstruktion-einer-bivariaten-normalverteilungen}
## Konstruktion einer bivariaten Normalverteilung
...
library(mvtnorm)
pdf(
file        = file.path("./_figures/311-konstruktion.pdf"),
width       = 3.5,
height      = 3.5)
par(
family      = "sans",
...
dev.off()
```
![Konstruktion einer bivariaten Normalverteilung.](./_figures/311-konstruktion){#fig-konstruktion fig-align="center" width=60%}


## Definition multivariater Normalverteilungen {#sec-definition-multivariater-normalverteilungen}
...
:::{#exm-bivariate-normalverteilungen}
## Bivariate Normalverteilungen

@fig-bivariate-normal-wdf A,B,C zeigen die Isokonturen der WDFen bivariat normalverteilter
...
library(mvtnorm)
pdf(
file        = file.path("./_figures/311-bivariate-normal-wdf.pdf"),
width       = 9,
height      = 3)
par(
family      = "sans",
...
dev.off()
```
![Wahrscheinlichkeitsdichtefunktionen bivariater Normalverteilungen.](./_figures/311-bivariate-normal-wdf){#fig-bivariate-normal-wdf fig-align="center" width=100%}
...
library(latex2exp)
pdf(
file        = file.path("./_figures/311-gemeinsame-normalverteilungen.pdf"),
width       = 9,
height      = 4.5)
par(
family      = "sans",
...
library(latex2exp)
pdf(
file        = file.path("./_figures/311-bedingte-normalverteilungen.pdf"),
width       = 10,
height      = 3.33)
par(
family      = "sans",
...
# Abbildungsparameter
pdf(
file        = file.path("./_figures/311-bivariate-normal-realisierungen.pdf"),
width       = 9,
height      = 3)
par(
family      = "sans",
...
![Realisierungen bivariat normalverteilter Zufallsvektoren.](./_figures/311-bivariate-normal-realisierungen){#fig-bivariate-normal-realisierungen fig-align="center" width=100%}
...
Wie im Falle der univariat normalverteilten Zufallsvariablen entspricht der
...
Kugeln bei $n = 3$). Eine multivariate Normalverteilung mit sphärischem Kovarianzmatrixparameter
nennt man entsprechend eine *sphärische Normalverteilung*. @thm-sphärizität besagt,
dass die WDF eines $n$-dimensionalen normalverteilten Zufallsvektors mit
...
nicht vorausgesetzt ist, insbesondere können sich ihre Erwartungswertparameter
$\mu_i, i = 1,...,n$ explizit unterscheiden.

## Marginale, bedingte und gemeinsame Verteilungen {#sec-marginale-bedingte-und-gemeinsame-verteilungen}
...
errechnet werden können. Speziell gilt zum einen, dass die uni- und multivariaten
...
Normalverteilungen, deren Parameter aus den Parametern der gemeinsamen Verteilung errechnet
...
:::{exm-marginale-normalverteilungen}
## Marginale Normalverteilungen
...
library(latex2exp)
pdf(
file        = file.path("./_figures/311-marginale-normalverteilungen.pdf"),
width       = 9,
height      = 4.5)
par(
family      = "sans",
...
p_marg   = list(dnorm(x_1, mu[1], sqrt(Sigma[1,1])), dnorm(x_1, mu[2], sqrt(Sigma[2,2])))
...
![Marginale Verteilungen eines bivariaten normalverteilten Zufallsvektor.](./_figures/311-marginale-normalverteilungen){#fig-marginale-normalverteilungen fig-align="center" width=100%}
...
Die Parameter der gemeinsamen Verteilung ergeben sich also als linear-affine
Transformationen der Parameter der induzierenden marginalen und bedingten Verteilungen.

:::{#exm-gemeinsame-normalverteilungen}
## Gemeinsame Normalverteilungen
...
![Gemeinsame Verteilungen einer marginalen und einer auf dieser bedingten normalverteilten Zufallsvariablen.](./_figures/311-gemeinsame-normalverteilungen){#fig-gemeinsame-normalverteilungen fig-align="center" width=100%}
...
:::{#exm-bedingte-normalverteilungen}
## Bedingte Normalverteilungen
...
![Bedingte Normalverteilungen](./_figures/311-bedingte-normalverteilungen){#fig-bedingte-normalverteilungen fig-align="center" width=100%}

## Multivariate Transformationen {#sec-multivariate-transformationen}
...
:::{#exm-invertierbare-lineare-transformation-einer-bivariaten-normalverteilung}
## Invertierbare lineare Transformation einer bivariaten Normalverteilung
...
zeigt die transformierten Realisierungen $z^{(i)} = Ax^{(i)} \in \mathbb{R}^{2}$
...
library(mvtnorm)
pdf(
file        = file.path("./_figures/311-lineare-transformation.pdf"),
width       = 8,
height      = 4)
par(
family      = "sans",
...
![Invertierbare lineare Transformation eines normalverteilten Zufallsvektors](./_figures/311-lineare-transformation){#fig-lineare-transformation fig-align="center" width=100%}

Die Tatsache, dass ein linear transformierter normalverteilter Zufallsvektor wiederum
...
## Transformationen normalverteilter Zufallsvariablen {#sec-normalverteilter-zufallsvariablen}

In diesem Abschnitt diskutieren wir sechs Transformationen normalverteilter
Zufallsvariablen, die in der Frequentistischen Inferenz zentrale Rollen spielen
und bei denen es sich um Anwendungen von den in @sec-transformationen eingeführten
Transformationstheoremen handelt. Unsere Aussagen sind dabei meist von der allgemeinen Form
...
für die Frequentistische Inferenz zentral, weil erstens die Zentralen Grenzwertsätze
die Annahme additiv unabhängig normalverteilter Störvariablen, und damit normalverteilter
Daten, begründet, zweitens, wie wir in @sec-frequentistische-inferenz
sehen werden, es sich bei Schätzern und Statistiken um Transformationen von
Zufallsvariablen handelt, und drittens, Parameterschätzergütekriterien,
Konfidenzintervalle und Hypothesentests der Frequentistischen Inferenz durch die
Verteilungen der jeweiligen Schätzer und Statistiken charakterisiert und begründet sind.

### Summationstransformation {-}

In diesem Abschnitt betrachten wir die resultierenden Verteilungen bei Summation
und Mittelwertbildung von unabhängig und identisch normalverteilten Zufallsvariablen.
Speziell besagt @thm-summationstransformation, dass die Summe
...
besagt, dass das Stichprobenmittel unabhängig normalverteilter Zufallsvariablen
wiederum normalverteilt ist und gibt die Parameter dieser Verteilung an.
...
sowie die in @eq-generalisierung-zentraler-grenzwertsatz-lindeberg-levy und
...
```{r, echo = FALSE, eval = FALSE}
library(latex2exp)
pdf(file =  "./_figures/311-summation.pdf", width = 12, height = 5)
par(
family     = "sans",
mfcol      = c(1,3),
pty        = "m",
...
![Summation normalverteilter Zufallsvariablen.](./_figures/311-summation){#fig-summation fig-align="center" width=100%}
...
in @eq-generalisierung-zentraler-grenzwertsatz-lindeberg-levy erwähnte Generalisierung
des Zentralen Grenzwertsatzes nach Lindeberg-Lévy. Wir visualisieren
@thm-mittelwerttransformation exemplarisch in @fig-mittelwert.

```{r, echo = FALSE, eval = FALSE}
# figure setup
library(latex2exp)
pdf(file = "./_figures/311-mittelwert.pdf", width = 12, height = 5)
par(
family      = "sans",
mfcol       = c(1,2),
pty         = "m",
...
![Mittelwertbildung bei normalverteilten Zufallsvariablen.](./_figures/311-mittelwert){#fig-mittelwert fig-align="center" width=100%}

### $Z$-Transformation {-}

Das @thm-z-transformation besagt, dass Subtraktion des Erwartungswertparameters
und gleichzeitige Division mit der Wurzel des Varianzparameters die Verteilung
...
= \frac{1}{\sigma}.
\end{equation}
Einsetzen in @thm-univariates-wdf-transformationstheorem-bei-linear-affinen-abbildungen
ergibt dann
\begin{align}
\begin{split}
p_Z(z)
...
# figure setup
library(latex2exp)
pdf(file = "./_figures/311-z-transformation.pdf", width = 5, height = 3)
par(
family     = "sans",
mfcol      = c(1,2),
pty        = "m",
...
![$Z$-Transformation normalverteilter Zufallsvariablen.](./_figures/311-z-transformation){#fig-z-transformation fig-align="center" width=100%}

### $\chi^2$-Transformation {-}

Mit der $\chi^2$-Transformation führen wir nun eine erste Transformation unabhängig
und identisch normalverteilter Zufallsvariablen ein, die *nicht* wiederum auf
...
$n$ sich  $\chi^2(u;n)$ verbreitert und Wahrscheinlichkeitsmasse zu größeren Werten
von $u$ verschoben wird.

```{r, eval = FALSE, echo = FALSE}
library(latex2exp)
pdf(file = "./_figures/311-chi2-wdf.pdf", width = 6,height = 4)
par(
family     = "sans",
pty        = "m",
bty        = "l",
...
![WDFen von $\chi^2$ Zufallsvariablen.](./_figures/311-chi2-wdf){#fig-chi2-wdf fig-align="center" width=80%}


```{r, echo = FALSE, eval = FALSE}
pdf(file = "./_figures/311-chi2-transform.pdf", width = 10, height = 5)
library(latex2exp)
par(
family     = "sans",
mfcol      = c(1,2),
...
```{r, echo = FALSE, eval = FALSE}
pdf(file  = "./_figures/311-t-wdf.pdf", width = 6, height = 5)
par(
family     = "sans",
pty        = "m",
bty        = "l",
...
```{r, eval = FALSE, echo = FALSE}
pdf(file = "./_figures/311-t-transformation.pdf", width = 6, height = 2)
library(latex2exp)
par(
family     = "sans",
mfcol      = c(1,3),
...
# Visualisierung
pdf(
file        = "./_figures/311-nichtzentrale-t-wdf.pdf",
width       = 7,
height      = 4.5)
library(latex2exp)
par(
...
library(latex2exp)
pdf(
file        = "./_figures/311-nichtzentrale-t-transformation.pdf",
width       = 7,
height      = 3)
par(
family      = "sans",
...
xlab        = "t",
ylab        = "",
main        = TeX("$T = y/\\sqrt{U/n} \\sim t(5,5)$"))
lines(
t,
p_T,
lwd         = 2,
...
```{r, echo = FALSE, eval = FALSE}
pdf(file = "./_figures/311-f-wdf.pdf", width = 6, height = 5)
par(
family     = "sans",
pty        = "m",
bty        = "l",
...
```{r, echo = FALSE, eval = FALSE}
pdf(file = "./_figures/311-f-transformation.pdf", width = 12, height = 4)
par(
family     = "sans",
mfcol      = c(1,3),
pty        = "m",
...
![$\chi^2$-Transformation normalverteilter Zufallsvariablen.](./_figures/311-chi2-transform){#fig-chi2-transform fig-align="center" width=100%}
...
in der ersten Hälfte der 20. Jahrhunderts. @hald2007 und @zabell2008 geben
...
![WDFen von $T$-Zufallsvariablen.](./_figures/311-t-wdf){#fig-t-wdf fig-align="center" width=60%}

:::{#thm-t-transformation}
## $T$-Transformation
$Z \sim N(0,1)$ sei eine standardnormalverteilte Zufallsvariable, $U \sim \chi^2(n)$
...
Linearen Modells. Wir visualisieren @thm-t-transformation exemplarisch in @fig-t-transformation.

![$T$-Transformation normalverteilter Zufallsvariablen.](./_figures/311-t-transformation){#fig-t-transformation fig-align="center" width=100%}
...
$\delta = 0$ einer $t$-Zufallsvariable entspricht, es gelten also
\begin{equation}
t(T;0,n) = t(T;n)
\end{equation}
sowie
...
Freiheitsgradparameter $n$ sich entsprechend lokalisierten Normalverteilungen mit
...
![WDFen von Nichtzentralen-$T$-Zufallsvariablen.](./_figures/311-nichtzentrale-t-wdf){#fig-nichtzentrale-t-wdf fig-align="center" width=80%}
...
![Nichtzentrale $T$-Transformation normalverteilter Zufallsvariablen.](./_figures/311-nichtzentrale-t-transformation){#fig-nichtzentrale-t-transformation fig-align="center" width=100%}

### $F$-Transformation {-}

Das in diesem Abschnitt zentrale @thm-f-transformation besagt, dass die Zufallsvariable,
die sich durch Division zweier jeweils durch ihre Freiheitsgradparameter geteilter $\chi^2$-verteilter Zufallsvariablen ergibt, eine $F$-verteilte Zufallsvariable ist.
...
Wir beobachten, dass die Form der WDFen zunächst primär durch den Freiheitsgradparameter
$n$ und dann sekundär durch den Freiheitsgradparameter $m$ bestimmt werden.

![WDFen von $F$-verteilten Zufallsvariablen.](./_figures/311-f-wdf){#fig-f-wdf fig-align="center" width=60%}

:::{#thm-f-transformation}
## $F$-Transformation
$V \sim \chi^2(n)$ und $W \sim \chi^2(m)$ seien zwei unabhängige
$\chi^2$-Zufallsvariablen mit Freiheitsgradparametern $n$ und $m$, respektive.
...
Das Theorem kann bewiesen werden, indem man zunächst ein Transformationstheorem
...
![$F$-Transformation normalverteilter Zufallsvariablen.](./_figures/311-f-transformation){#fig-f-transform fig-align="center" width=100%}
...
geht dabei wohl auf @pearson1896 zurück (@seal1967). Die ursprüngliche Formulierung
der multivariaten Normalverteilung wird bei @edgeworth1892 verortet. @tong1990
gibt einen umfassenden Überblick zur Theorie und Anwendung der multivariaten Normalverteilung.
...
1. Geben Sie das Mittelwerttransformationstheorem wieder.
1. Geben Sie das $Z$-Transformationstheorem wieder.
1. Geben Sie das $\chi^2$-Transformationstheorem wieder.
1. Beschreiben Sie die WDF der $t$-Verteilung in Abhängigkeit ihres Freiheitsgradparameters.
1. Geben Sie das $T$-Transformationstheorem wieder.
1. Geben Sie das $F$-Transformationstheorem wieder.

## Lösungen {.unnumbered .unlisted}

1. Siehe @thm-summationstransformation.
1. Siehe @thm-mittelwerttransformation.
1. Siehe @thm-z-transformation.
1. Siehe @thm-chi2-transformation.
1. Siehe @fig-t-wdf.
1. Siehe @thm-t-transformation.
1. Siehe @thm-f-transformation.
