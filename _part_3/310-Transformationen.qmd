...
Die Transformation von Zufallsvektoren und die Analyse ihrer resultierenden 
Verteilungen ist ein zentrales Thema der probabilistischen Modellierung und in 
besonderem Maße der Frequentistischen Inferenz. Mit einer *Transformation* soll 
hier die Anwendung einer Funktion auf einen Zufallsvektor gemeint sein, die seine 
Komponenten entweder individuell transformiert oder miteinander verknüpft.
Die zentrale Fragestellung dabei ist folgende: "Wenn ein Zufallsvektor $\xi$ 
eine vorgegebene Verteilung hat, wie ist dann der Zufallsvektor $\upsilon$, der
sich durch Transformation von $\xi$ ergibt, verteilt?" Für die in diesem Kapitel 
behandelten Fälle gilt dabei, dass man explizite Lösungen für die Verteilung der 
transformierten Zufallsvektoren angeben kann. Diese gehören zu den klassischen 
Resultaten der Frequentistischen Inferenz und sind für das Verständnis von 
klassischer Frequentistischer Inferenzverfahren wie Konfidenzintervallen und 
Hypothesentests essenziell.

Für eine erste Intuition hierzu beschränken wir uns auf den Fall einer einzelnen 
Zufallsvariable. Man kann sich dann die Transformation einer Zufallsvariable 
anhand der Transformation ihrer unabhängig und identisch verteilten Realisierungen 
verdeutlichen. Betrachtet man beispielsweise die Zufallsvariable $\xi \sim N(0,1)$ und ihre 
...
die Verteilung von $\upsilon$ ordnet negativen Werten daher eine Wahrscheinlichkeitsdichte 
von $0$ zu. Untenstehender **R** Code simuliert diese Überlegungen. @fig-hist-x zeigt das
...
# Histogramm von \xi
pdf(file = "./_figures/310-hist-x.pdf", width = 7, height = 5)
library(latex2exp)
hist(
x,
...
dev.off()
```
![Histogramm von 10.000 Realisierungen unabhängig und identisch normalverteilter Zufallsvariablen.](./_figures/310-hist-x){#fig-hist-x fig-align="center" width=80%}

```{r, echo = FALSE, eval = FALSE}
# Histogramm von \upsilon
pdf(file = "./_figures/310-hist-y.pdf", width = 7, height = 5)
hist(
y,
...
![Histogramm von 10.000 quadrierten Realisierungen unabhängig und identisch normalverteilter Zufallsvariablen.](./_figures/310-hist-y){#fig-hist-y fig-align="center" width=80%}
...
Funktion auf eine zufällige Größe im Allgemeinen wieder eine zufällige Größe ergibt. 
In einem Beweis von @thm-transformation-eines-zufallsvektors müsste die Messbarkeit 
von $\upsilon$ als Folge der Messbarkeit von $\xi$ nachgewiesen werden. Im Folgenden 
ist oft $\mathcal{X} := \mathbb{R}$ und $f : \mathbb{R} \to \mathbb{R}$. Wir schreiben 
in diesem Fall in der Regel einfach $\upsilon := f(\xi)$ und nennen $\upsilon$ 
die *transformierte Zufallsvariable*. 

In @sec-univariate-transformationstheoreme betrachten wir Theoreme zur 
Transformation von Zufallsvariablen in Abhängigkeit von der Art der betrachteten 
Funktion $f$. In @sec-multivariate-transformationstheoreme betrachten wir ein 
Theorem zur Transformation von Zufallsvektoren bei bijektiver Transformationsfunktion
und die Bestimmung von Verteilungen bei der binären Verknüpfungen von Zufallsvariablen.

## Univariate Transformationstheoreme {#sec-univariate-transformationstheoreme}

@thm-univariate-wdf-transformation-bei-bijektiven-abbildungen gibt eine Formel
zur Berechnung der WDF $p_\upsilon$ von $\upsilon := f(\xi)$, wenn $\xi$ eine
...
Ein wichtiger Anwendungsfall von @thm-univariate-wdf-transformation-bei-bijektiven-abbildungen ist @thm-univariates-wdf-transformationstheorem-bei-linear-affinen-abbildungen. Im Vergleich
zu @thm-univariate-wdf-transformation-bei-bijektiven-abbildungen gibt 
...
wenn $f$ eine linear-affine Funktion ist. 
...
@thm-univariate-wdf-transformation-bei-stückweise-bijektiven-abbildungen gibt 
eine Formel zur Berechnung der WDF $p_\upsilon$ von $\upsilon := f(\xi)$ an,
...
gelten muss), sodass die Abbildung $f$ für alle $\mathcal{X}_1,...,\mathcal{X}_k$
bijektiv ist (d.h. $f$ ist eine *stückweise* bijektive Abbildung). Für
...
Für einen Beweis verweisen wir auf @casella2002.

## Multivariate Transformationstheoreme {#sec-multivariate-transformationstheoreme}
...
& \mbox{ für } y\in f(\mathbb{R}^n) \\
0
& \mbox{ für } y \in \mathbb{R}^n \setminus f(\mathbb{R}^n)
\end{cases}
\end{equation}
gegeben.
:::

Für einen Beweis verweisen wir auf @billingsley1995.
...
\end{equation}
Die Formel für die WDF $p_\upsilon$ heißt *Konvolution* oder *Faltung* von 
$p_{\xi_1}$ und $p_{\xi_2}$.
:::
...
dass die Unabhängigkeit von $\xi_1$ und $\xi_2$ impliziert, dass
\begin{equation}
p_{\xi_1,\xi_2}(x_1,x_2) = p_{\xi_1}(x_1)p_{\xi_2}(x_2)
\end{equation}
impliziert. Einsetzen und Integration hinsichtlich $x_2$ ergibt dann  
...
\end{split}
\end{align}
Mit $\zeta_1 = \xi_1 + \xi_2 = \upsilon$ ergibt sich dann die erste Form des 
Konvolutionstheorems zu
\begin{align}
p_\upsilon(y)
& = \int_{-\infty}^{\infty} p_{\xi_1}(y - x_2)p_{\xi_2}(x_2)\,dx_2.
\end{align}
...
\footnotesize
1. Erläutern Sie das Konzept der Transformation einer Zufallsvariable am Beispiel von $\upsilon := \xi^2$.

## Lösungen {.unnumbered .unlisted}

1. Siehe Erläuterungen zu @fig-hist-x und @fig-hist-y.
