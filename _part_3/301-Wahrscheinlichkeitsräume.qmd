...
quantitativen Schlüssen über Zufallsvorgänge führt. In diesem Kapitel führen wir 
den Begriff des Wahrscheinlichkeitsraums ein (@sec-definition-und-erste-eigenschaften)
und geben dann mithilfe von Wahrscheinlichkeitsfunktionen (@sec-wahrscheinlichkeitsfunktionen) 
erste Beispiele für die Modellierung von Zufallsvorgängen (@sec-beispiele-bei-endlichem-ergebnisraum).   
...
des Werfens eines Würfels könnte man zum Beispiel $\Omega := \{1,2,3,4,5,6\}$ definieren.
...
genau dann eintritt, wenn in einem Durchgang des Zufallsvorgangs *Werfen eines Würfels* 
...
also auch lesen als "In einem Durchgang des Zufallsvorgangs *Werfen eines Würfels* 
wird ein Element von $\{3,4,5,6\}$ realisiert", da heißt, konkret fällt entweder eine
Drei, eine Vier, eine Fünf oder eine Sechs. Natürlich sind auch die Ergebnisse 
$\omega \in \Omega$ selbt mögliche Ereignisse, sodass zum Beispiel folgende 
...
eintretende Ereignisse keine Wahrscheinlichkeiten angeben könnte. Das Ereignissystem
...
Bei der Modellierung des Werfens eines Würfels soll also zum Beispiel 
...
$A_1,A_2,... \in \mathcal{A}$ folgen soll, dass auch $\cup_{i=1}^\infty A_i \in \mathcal{A}$ ist.
...
Zufallsvorgängen meist keine große Herausforderung dar, da sowohl für endliche
...
erfüllt. Sei nun $A$ eine Teilmenge von $\Omega$. Dann ist auch $A^c = \Omega \setminus A$ 
eine Teilmenge von $\Omega$ und somit ist auch die zweite $\sigma$-Algebra
...
Ereignissystems komplexer, sodass wir in dieser Hinsicht für formale Entwicklungen
auf die weiterführende Literatur verweisen wollen (vgl. @meintrup2005, @schmidt2009).
...
und formt damit die *funktionelle Basis* eines solchen. Wir werden im Folgenden, 
insbesondere nach Einführung der Begriffe der Zufallsvariablen und Zufallsvektoren, 
sehr viele verschiedene Wahrscheinlichkeitsmaße kennenlernen. An dieser Stelle 
wollen wir zunächst nur allgemeine Eigenschaften von Wahrscheinlichkeitsmaßen betrachten.
...
weitere. Beim Modell des Werfens eines Würfels gilt also, dass das in einem Durchgang
...
rational, also nach den Regeln der Vernunft, das heißt der Logik und der Mathematik, bestimmt. 
Insgesamt erlaubt das Wahrscheinlichkeitsmodell damit das schlussfolgernde 
Nachdenken über Zufallsvorgänge, also mit Unsicherheit behaftete Phänomene.
...
disjunkter Ereignisse, wie sie in der Anwendung oft vorkommen, impliziert.
...
Zur Vorbereitung des Begriffs der gemeinsamen Wahrscheinlichkeit von Ereignissen
wollen wir abschließend noch eine weitere wichtige Eigenschaft von $\sigma$-Algebren 
betrachten: neben der Abgeschlossenheit von $\mathcal{A}$ unter der Bildung von 
abzählbaren Vereinigungen ist $\mathcal{A}$ auch unter der Bildung von 
abzählbaren Durchschnitten abgeschlossen. Dies ist eine direkte Konsequenz der 
Abgeschlossenheit von $\mathcal{A}$ unter der Bildung von Komplementärmengen 
und abzählbaren Vereinigungen und wird in folgendem Theorem festgehalten.

:::{#thm-abgeschlossenheit-von-sigma-algebren-bezüglich-durchschnitten}
## Abgeschlossenheit von $\sigma$-Algebren bezüglich Durchschnitten
$\mathcal{A}$ sei eine $\sigma$-Algebra auf einer Menge $\Omega$. Dann folgt aus 
$A,B \in \mathcal{A}$, dass auch $A \cap B \in \mathcal{A}$. Allgemein gilt 
für $A_1,A_2,... \in \mathcal{A}$, dass auch $\cap_{i=1}^\infty A_i \in \mathcal{A}$.
::: 

:::{.proof}
Wir zeigen exemplarisch die erste Aussage. Dazu halten wir zunächst fest, dass mit 
der ersten De Morganschen Regel und den Eigenschaften von Komplementen gilt, dass
\begin{equation}
(A \cap B)^c = A^c \cup B^c 
\Leftrightarrow
\left((A \cap B)^c \right)^c = \left(A^c \cup B^c \right)^c
\Leftrightarrow A \cap B = \left(A^c \cup B^c \right)^c.
\end{equation}
Ferner gilt mit den definierenden Eigenschaften von $\sigma$-Algebren, dass 
für $A,B \in \mathcal{A}$ gilt, dass auch $\left(A^c \cup B^c\right)^c \in \mathcal{A}$.
Denn aus der Abgeschlossenheit von $\mathcal{A}$ unter der Bildung von 
Komplementärmengen folgt zunächst, dass auch $A^c \in \mathcal{A}$ und $B^c \in \mathcal{A}$. 
Weiterhin folgt aus der Abgeschlossenheit von $\mathcal{A}$ unter der 
abzählbaren Vereinigung auch $A^c \cup B^c \in \mathcal{A}$. Mit der Abgeschlossenheit 
von $\mathcal{A}$ unter der Bildung von Komplementärmengen folgt schließlich
$\left(A^c \cup B^c\right)^c \in \mathcal{A}$. Insgesamt gilt also, dass aus 
$A,B \in \mathcal{A}$ folgt, dass $A \cap B \in \mathcal{A}$. 
Die Verallgemeinerung auf abzählbare Durchschnitte $A_1,A_2,... \in \mathcal{A}$    
::: 

Nach @thm-abgeschlossenheit-von-sigma-algebren-bezüglich-durchschnitten ist neben den
Wahrscheinlichkeiten $\mathbb{P}(A), \mathbb{P}(B)$ und $\mathbb{P}(A \cup B)$ 
mit $A,B \in \mathcal{A}$ also auch die Wahrscheinlichkeit $\mathbb{P}(A \cap B)$ 
wohldefiniert.

## Wahrscheinlichkeitsfunktionen {#sec-wahrscheinlichkeitsfunktionen}
In diesem Abschnitt wollen wir mit den *Wahrscheinlichkeitsfunktionen* eine 
Möglichkeit kennenlernen, für Wahrscheinlichkeitsräume mit endlicher Ergebnismenge
Wahrscheinlichkeitsmaße festzulegen. In @sec-zufallsvariablen und @sec-zufallsvektoren 
werden wir Wahrscheinlichkeitsfunktionen unter der Bezeichnung *Wahrscheinlichkeitsmassefunktionen* 
erneut begegnen. Wir definieren hier den Begriff der Wahrscheinlichkeitsfunktion 
zunächst wie folgt.
...
Wir merken an, dass weil $\mathbb{P}$ definitionsgemäß $\sigma$-additiv ist, 
...
$\pi : \Omega \to [0,1]$ und stellt sicher, dass sich ihre Funktionswerte $\pi(\omega)$ 
...
Wahrscheinlichkeitsmaß konstruiert. Definiert man andersherum bei einer endlichen
Ergebnismenge $\Omega$ für alle Elementarereignisse $\{\omega\} \in \mathcal{A}$ 
eine Wahrscheinlichkeit $\mathbb{P}\left(\{\omega\}\right) \in [0,1]$ und stellt 
icher, dass $\mathbb{P}(\Omega) = 1$ gilt, so erfüllt man implizit natürlich 
auch die Anforderungen an eine Wahrscheinlichkeitsfunktion. Man kann in diesem Fall 
die $\sigma-$-Additivität von $\mathbb{P}$ für die paarweise disjunkten Elementareignisse 
direkt zur Berechnung zusammengesetzter Ereignisse nutzen. Wir verdeutlichen dieses und 
das Vorgehen mithilfe einer Wahrscheinlichkeitsfunktion in @sec-beispiele-bei-endlichem-ergebnisraum
durch Beispiele.
...
(2) In einem zweiten Schritt wählt man dann ein geeignetes Ereignissystem. Im Fall
einer endlichen Ergebnismenge bietet sich die Potenzmenge $\mathcal{P}(\Omega)$ an,
im Fall der überabzählbaren Ergebnismenge $\Omega := \mathbb{R}$ bietet sich die 
...
dieses Vorgehen anhand von Beispielen. Für die überabzählbare Ergebnismenge 
...
Da es sich um eine endliche Ergebnismenge handelt, wählen wir als $\sigma$-Algebra 
$\mathcal{A}$ die Potenzmenge $\mathcal{P}(\Omega)$. $\mathcal{A}$ enthält 
dann automatisch alle möglichen Ereignisse. Die Kardinalität von 
$\mathcal{A} := \mathcal{P}(\Omega)$ ist $|\mathcal{P}(\Omega)| = 2^{|\Omega|} = 2^6 = 64$.
In @tbl-wuerfel listen wir sechs dieser 64 Ereignisse in ihrer verbalen Beschreibung 
und als Teilmenge $A$ von $\Omega$ auf.
...
: Ausgewählte Ereignisse beim Modell des Werfens eines Würfels. {#tbl-wuerfel}
...
Wie in @sec-wahrscheinlichkeitsfunktionen beschrieben, kann ein Wahrscheinlichkeitsmaß 
$\mathbb{P}$ durch Festlegung von $\mathbb{P}(\{\omega\})$ für alle $\omega \in \Omega$ 
definiert werden. Für das Modell eines unverfälschten Würfels würde man
\begin{equation}
...
\mathbb{P}(\{6\}) := \frac{3}{8}.
\end{equation}
Im Fall des unverfälschten Würfels ergibt sich zum Beispiel die Wahrscheinlichkeit 
...
= \frac{3}{6}.
\end{equation}
Im Fall des obigen Modells eines verfälschten Würfels ergibt sich die 
Wahrscheinlichkeit für das gleiche Ereignis dagegen zu
\begin{equation}
\mathbb{P}(\{2,4,6\})
= \mathbb{P}(\{2\} \cup \{4\} \cup \{6\})
= \mathbb{P}(\{2\}) + \mathbb{P}(\{4\}) + \mathbb{P}(\{6\})
...
Wir modellieren das zweifache Werfen einer Münze. Basierend auf dem Modell des
...
Wir modellieren den Wert einer Patient:in für das Item "Traurigkeit" der deutschen
Version des Beck Depression Inventory II (@hautzinger2006),

(0) Ich bin nicht traurig.
(1) Ich bin oft traurig.
(2) In bin ständig traurig.
(3) In bin so traurig oder unglücklich, dass ich es nicht aushalte.

Der betrachtete Zufallsvorgang sei also die Antwort einer Patient:in für dieses Item.
Dabei bietet sich zur Modellierung dann die Ergebnismenge $\Omega := \{0,1,2,3\}$ an. 
Für eine erste Patient:in ergibt sich beispielsweise die Realisierung $3 \in \Omega$, 
für eine zweite Patient:in die Realisierung $0 \in \Omega$, für eine dritte Patient:in
$1 \in \Omega$ und so weiter. Als Ereignissystem $\mathcal{A}$ bietet sich wiederum
die Potenzmenge an. Diese hat im vorliegenden Fall die Kardinalität 
\begin{equation}
|\mathcal{P}(\Omega)| = 2^{|\Omega|} = 2^4 = 16
\end{equation}
und kann wie in @tbl-bdi-item gezeigt leicht vollständing hingeschrieben werden. 

\footnotesize

| Angegebene Antwort                                                                                | Mengenform    |
|:--------------------------------------------------------------------------------------------------|:--------------|
| Keine                                                                                             | $\emptyset$   |
| "Ich bin nicht traurig"                                                                           | $\{0\}$       |
| "Ich bin oft traurig"                                                                             | $\{1\}$       |
| "Ich bin ständig traurig"                                                                         | $\{2\}$       |
| "Ich bin so traurig oder unglücklich, dass ich es nicht aushalte"                                 | $\{3\}$       |
| "Ich bin nicht traurig" oder "Ich bin oft traurig"                                                | $\{0,1\}$     |
| "Ich bin nicht traurig" oder "Ich bin ständig traurig"                                            | $\{0,2\}$     |
| "Ich bin nicht traurig" oder "Ich bin so traurig oder unglücklich, dass ich es nicht aushalte"    | $\{0,3\}$     |
| "Ich bin oft traurig" oder "Ich bin ständig traurig"                                              | $\{1,2\}$     |
| "Ich bin oft traurig" oder "Ich bin so traurig oder unglücklich, dass ich es nicht aushalte"      | $\{1,3\}$     |
| "Ich bin ständig traurig" oder "Ich bin so traurig oder unglücklich, dass ich es nicht aushalte"  | $\{2,3\}$     |
|  Nicht "Ich bin so traurig oder unglücklich, dass ich es nicht aushalte"                          | $\{0,1,2\}$   |
|  Nicht "Ich bin ständig traurig"                                                                  | $\{0,1,3\}$   |
|  Nicht "Ich bin oft traurig"                                                                      | $\{0,2,3\}$   |
|  Nicht "Ich bin nicht traurig"                                                                    | $\{1,2,3\}$   |
|  Eine beliebige Antwortmöglichkeit                                                                | $\{0,1,2,3\}$ |

: Ausgewählte Ereignisse beim Modell der Beantwortung eines BDI-II Items. {#tbl-bdi-item}

\normalsize


Gehen wir nun davon aus, dass die Wahrscheinlichkeiten der Itemantworten als
\begin{equation}
\mathbb{P}(\{0\}) := \frac{2}{10},\quad
\mathbb{P}(\{1\}) := \frac{3}{10},\quad
\mathbb{P}(\{2\}) := \frac{4}{10},\quad
\mathbb{P}(\{3\}) := \frac{1}{10}
\end{equation}
bekannt sind. Dann definieren diese Wahrscheinlichkeiten im Sinne einer 
Wahrscheinlichkeitsfunktion 
\begin{equation}
\pi: \Omega \to [0,1], \omega \mapsto \pi(\omega) =: \mathbb{P}(\{\omega\})
\end{equation}
ein Wahrscheinlichkeitsmaß und die Definition des Wahrscheinlichkeitsraummodells 
$(\Omega, \mathcal{A},\mathbb{P})$ für das betrachtete Szenario ist abgeschlossen.
Mithilfe der $\sigma$-Addivität von $\mathbb{P}$ lässt sich nun beispielsweise
die Wahrscheinlichkeit für eine Itemantwort größer als 1 bestimmen:
\begin{equation}
\mathbb{P}\left(\{2,3\}\right) 
= \mathbb{P}\left(\{2\} \cup \{3\}\right) 
= \mathbb{P}\left(\{2\}\right) + \mathbb{P}\left(\{3\}\right)
= \frac{4}{10} + \frac{1}{10}
= \frac{5}{10}
= \frac{1}{2}
\end{equation}
Ebenso ergibt sich beispielsweise die Wahrscheinlichkeit dafür, dass eine Patient:in
eine der beiden Itemantworten "(0) Ich bin nicht traurig" oder "(3) In bin 
so traurig oder unglücklich, dass ich es nicht aushalte" angibt, zu 
\begin{equation}
\mathbb{P}\left(\{0,3\}\right) 
= \mathbb{P}\left(\{0\} \cup \{3\}\right) 
= \mathbb{P}\left(\{0\}\right) + \mathbb{P}\left(\{3\}\right)
= \frac{2}{10} + \frac{1}{10}
= \frac{3}{10}.
\end{equation}
...
Schließlich ist die Entwicklung der mathematischen Modellierung von Zufallvorgängen 
auch mit @kolmogoroff1933 keinesfalls an einem Ende angelangt. Spätere Arbeiten im 20. Jahrhundert betrafen
...
1. Welche $\sigma$-Algebra wählt man sinnvollerweise für einen Wahrscheinlichkeitsraum mit endlicher Ergebnismenge?
...
1. Warum ist der Begriff der Wahrscheinlichkeitsfunktion bei der Modellierung eines Zufallsvorgans durch einen Wahrscheinlichkeitsraum mit endlicher Ergebnismenge hilfreich?
...
1. Ein Zufallsvorgang ist ein Phänomen der Wirklichkeit, das nicht mit absoluter
Sicherheit vorhersagbar ist bzw. für Menschen mit Unsicherheit behaftet ist. 
Siehe Einleitung zur Wahrscheinlichkeitstheorie.
1. Siehe @def-wahrscheinlichkeitsraum bezüglich $\mathcal{A}$.
1. Siehe @def-wahrscheinlichkeitsraum bezüglich $\mathbb{P}$.
1. Siehe @def-wahrscheinlichkeitsraum.
1. Die Ergebnismenge enthält die in einem Durchgang des modellierten Zufallsvorgangs möglichen Elementarereignisse, siehe die Erläuterungen zu **Ergebnismenge und Mechanik**.
1. Siehe die Erläuterungen zu **Ergebnismenge und Mechanik**.
1. Siehe die Erläuterungen zu **Ereignisse und Ereignissysteme**.
1. Siehe die Erläuterungen zu **Ereignisse und Ereignissysteme**.
1. Die Potenzmenge der Ergebnismenge, sie enthält alle prinzipiell möglichen Ereignisse, siehe auch @thm-ereignissystem-bei-endlicher-ergebnismenge.
1. Siehe die Erläuterungen zu **Wahrscheinlichkeitsmaß $\mathbb{P}$**.
1. Siehe @def-wahrscheinlichkeitsfunktion.
1. Durch Definition der Funktionswerte einer Wahrscheinlichkeitsfunktion kann ein Wahrscheinlichkeitsmaß festgelegt werden, siehe auch @sec-beispiele-bei-endlichem-ergebnisraum.
1. Siehe die Erläuterungen zum **Würfeln mit einem Würfel**.
1. Siehe die Erläuterungen zum **Gleichzeitigen Würfeln mit einem blauen und einem roten Würfel**.
