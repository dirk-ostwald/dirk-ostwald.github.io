...
(vgl. @thm-ueberschreitungswahrscheinlichkeit) und dem Erwartungswert einer 
...
sind bekanntlich definitionsgemäß nicht-negativ (vgl. @def-gamma-zufallsvariable)
Für den Erwartungswert einer Gamma-Zufallsvariable gilt $\mathbb{E}(\xi) = \alpha\beta$. 
Wir betrachten konkret den Fall $\alpha := 5$ und $\beta := 2$, sodass $\xi$ 
...
# Abbildungsparameter
library(latex2exp)
pdf(file = "./_figures/308-markov-ungleichung.pdf", width=8, height=4)                                                                 
par(                                                                    
family     = "sans",                                                  
mfcol      = c(1,2),                                                 
pty        = "m",                                                    
...
![Markov-Ungleichung am Beispiel einer Gamma-Zufallsvariable.](./_figures/308-markov-ungleichung){#fig-markov-ungleichung fig-align="center" width=100%}

Die Chebychev Ungleichung setzt die Wahrscheinlichkeit dafür, dass eine Zufallsvariable
Werte weit von ihrem Erwartungswert entfernt annimmt, in Bezug zu ihrer Varianz.
Wie in @sec-varianzen erläutert, liefert die Chebychev Ungleichung damit eine 
Begründung dafür, warum die in @def-varianz-und-standardabweichung formulierte Größe 
als ein Maß für die Variabilität einer Zufallsvariable verstanden werden kann. 
Im Beweis der Chebyshev -Ungleichung wird an entscheidender Stelle auf die Markov 
Ungleichung zurückgegriffen.
...
\end{equation}
(2) $a^2 \ge b^2$ für $a < 0$ und $b \ge 0$. Dann gilt
\begin{equation}
a^2 \ge b^2 \Rightarrow \sqrt{a^2} \ge \sqrt{b^2} \Rightarrow -a \ge b \Rightarrow |a| \ge b.
\end{equation}
(3) $a^2 \ge b^2$ für $a \ge 0$ und $b < 0$. Dann gilt
\begin{equation}
a^2 \ge b^2 \Rightarrow \sqrt{a^2} \ge \sqrt{b^2} \Rightarrow a \ge -b \ge b \Rightarrow |a| \ge b.
\end{equation}
(4) $a^2 \ge b^2$ für $a < 0$ und $b < 0$. Dann gilt
\begin{equation}
a^2 \ge b^2 \Rightarrow \sqrt{a^2} \ge \sqrt{b^2} \Rightarrow -a \ge -b > b \Rightarrow |a| \ge b.
\end{equation}
...
Analog zu @thm-cauchy-schwarz-ungleichung gilt zum Beispiel für Vektoren $x,y \in \mathbb{R}^n$, dass 
\begin{equation}
...
$\xi$ und $\upsilon$ seien Zufallsvariablen mit $\mathbb{V}(\xi) > 0$ und $\mathbb{V}(\upsilon) > 0$. Dann gelten
\begin{equation}
...
Dann gilt aber auch
\begin{equation}
|\rho(\xi,\upsilon)| \le 1 \Leftrightarrow -1 \le \rho(\xi,\upsilon) \le 1,
\end{equation}
denn
\begin{equation}
\rho(\xi,\upsilon)^2 \le 1 \Rightarrow \sqrt{\rho(\xi,\upsilon)^2} \le \sqrt{1}
\Rightarrow \quad\rho(\xi,\upsilon)  \le 1 \Rightarrow |\rho(\xi,\upsilon)| \le 1 \mbox{ für } \rho(\xi,\upsilon) \ge 0
\end{equation}
und 
\begin{equation}
\rho(\xi,\upsilon)^2 \le 1 \Rightarrow \sqrt{\rho(\xi,\upsilon)^2} \le \sqrt{1}
\Rightarrow  -\rho(\xi,\upsilon) \le 1 \Rightarrow |\rho(\xi,\upsilon)| \le 1 \mbox{ für } \rho(\xi,\upsilon) < 0.
\end{equation}
:::
...
also immer Werte zwischen -1 und 1 inklusive annimmt (vgl. @sec-kovarianzen).
...
der Funktionsgraf von $g$ über einem Intervall $[x_1,x_2]$ immer unter der 
...
```{r, eval = FALSE, echo = FALSE}
# figure setup
pdf(file = "_figures/308-konvexe-funktion.pdf", width=5, height=5)   
library(latex2exp)                                                                       
par(
family     = "sans",                                                  
mfcol      = c(1,1),                                                  
...
dev.off()
```
![Darstellung der konvexen Funktion $g(x) := x^2$ mit $x_1 := -\sqrt{1.5}, x_2 := \sqrt{3.5}, \lambda \in [0,1]$ und $x_0 := 1$.](./_figures/308-konvexe-funktion){#fig-konvexe-funktion fig-align="center" width=80%}
...
1. Siehe @thm-markov-ungleichung.
1. Siehe @thm-chebyshev-ungleichung.
1. Siehe @thm-cauchy-schwarz-ungleichung.
1. Siehe @thm-korrelationsungleichung.