...
\end{equation}
:::
Das Standardbeispiel für den Ergebnisraum eines Zufallsvektors ist $\mathbb{R}^n$,
...
uns hier wieder mit der (weiterhin formal falschen) Intuition der $n$-dimensionalen Borelschen
...
Kontext probabilistischer Standardmodelle wie dem Allgemeinen linearen Modell aber
durchaus üblich, sodass wir hier den Begriff des Zufallsvektors bevorzugen 
...
![Konstruktion von Zufallsvektor und multivariater Verteilung.](./_figures/304-zufallsvektor){#fig-zufallsvektor fig-align="center" width=80%}
...
Eine entsprechende Funktion $p$ heißt *multivariate Wahrscheinlichkeitsmassefunktion (WMF)* von $\xi$.
:::
...
: Bivariate WMF $p(x_1,x_2)$ {#tbl-bivariate-wmf}
...
Offenbar ist der Begriff der multivariaten WDF eines kontinuierlichen Zufallsvektors 
...
die wir in @sec-normalverteilungen ausführlich diskutieren.
...
:::{#exm-marginale-wahrscheinlichkeitsmassefunktionen}
## Marginale Wahrscheinlichkeitsmassefunktionen

In Fortführung von @exm-diskreter-zufallsvektor ergeben sich für die dort definierte 
WMF des Zufallsvektors $\xi:= (\xi_1,\xi_2)$ für die marginalen WMFen die an den 
Rändern der  von @tbl-bivariate-wmf-marginal aufgelisteten WMFen anhand von 
\begin{equation}
...
: Bivariate WMF $p(x_1, x_2)$ mit marginalen WMFen {#tbl-bivariate-wmf-marginal}
...
am Standardbeispiel der multivariaten Normalverteilung in @sec-normalverteilungen.

## Bedingte Verteilungen {#sec-bedingte-verteilungen}

Hat man die Verteilung eines Zufallsvektors spezifiziert, so kann man sich fragen,
welche Verteilung daraus für eine einzelne Komponente des Zufallsvektors folgt, 
...
\end{equation}
Die bedingten Verteilungen mit WMFen $p(x_1|x_2)$ und $p(x_2|x_1)$ heißen dann 
...
:::{#exm-bedingte-wahrscheinlichkeitsmassefunktionen}
## Bedingte Wahrscheinlichkeitsmassefunktionen

In Fortführung des in @exm-diskreter-zufallsvektor betrachteten
Beispiels eines zweidimensionalen Zufallsvektors $\xi:= (\xi_1,\xi_2)$ und seiner
in @exm-marginale-wahrscheinlichkeitsmassefunktionen bestimmten Marginalverteilungen ergeben sich die 
in @tbl-bedingte-wmf-x1-g-x2 dargestellten bedingte WMFen für $p(x_1|x_2)$ und die
in @tbl-bedingte-wmf-x2-g-x1 dargestellten bedingte WMFen für $p(x_2|x_1)$. 
...
: Bedingte WMFen $p(x_1|x_2)${#tbl-bedingte-wmf-x1-g-x2}
...
: Bedingte WMFen $p(x_2 | x_1)$ {#tbl-bedingte-wmf-x2-g-x1}
...
Die Verteilungen mit WDFen $p$ und $p$ heißen danndie *kontinuierlichen bedingten 
Verteilungen von $\xi_1$ gegeben $\xi_2 = x_2$ und $\xi_2$ gegeben $\xi_1 = x_1$*, respektive.
:::
...
Ähnlich wie die bedingten Wahrscheinlichkeiten von Ereignissen lässt sich auch
...
:::{#def-unabhaengige-zufallsvariablen}
...
@def-unabhaengige-zufallsvariablen besagt, dass die Ereignisse $\{\xi_1 \in S_1\}$ 
...
WDFen von Zufallsvektoren. Dies ist die Aussage folgenden Theorems. 
...
@thm-unabhängigkeit-und-faktorisierung ist für weite Aspekte der probabilistischen 
Modellierung grundlegend.

:::{#exm-unabhängige-wahrscheinlichkeitsmassefunktionen}
## Unabhängige Wahrscheinlichkeitsmassefunktionen

Wir betrachten erneut den zweidimensionalen Zufallsvektor $\xi:= (\xi_1, \xi_2)$
aus @exm-diskreter-zufallsvektor, dessen gemeinsame und marginale WMFen die in 
@tbl-bivariate-wmf-marginal spezifierten Formen haben. Wir fragen zunächst, ob 
$\xi_1$ und $\xi_2$ unabhängig sind. Dies ist nicht 
...
bei gleichen Marginalverteilungen wie in @tbl-bivariate-wmf-marginal ergibt sich
zu der in @tbl-gemeinsame-wmf-marginal-unabhaengig gemeinsamen WMF.
...
: Bivariate WMF bei Annahme der Unabhängigkeit von $\xi_1$ und $\xi_2$ {#tbl-gemeinsame-wmf-marginal-unabhaengig}


Weiterhin ergeben sich im Falle der Unabhängigkeit von $\xi_1$ und $\xi_2$ beispielsweise
die bedingten WMFen $p(x_2|x_1)$ wie in @tbl-bedingte-wmf-unabhaengig dargestellt.
...
: Bedingte WMF bei Annahme der Unabhängigkeit von $\xi_1$ und $\xi_2$ {#tbl-bedingte-wmf-unabhaengig}
:::

Wir wollen abschließend den Begriff der unabhängigen Zufallsvariablen für mehr 
als zwei Zufallsvariablen definieren.

:::{#def-n-unabhaengige-zufallsvariablen}
...
Es handelt bei @def-n-unabhaengige-zufallsvariablen also um eine direkte 
Generalisierung des zweidimensionalen Falls. Sind $n$ Zufallsvariablen nicht 
nur unabhängig, sondern haben sie auch alle die gleiche Verteilung, so nennt 
man sie *unabhängig und identisch verteilt (u.i.v.)*.

:::{#def-unabhaengig-und-identisch-verteilte-zufallsvariablen}
...
modelliert. Schließlich halten wir fest, dass $n$ u.i.v. normalverteilte 
Zufallsvariablen als
\begin{equation}
\xi_1,...,\xi_n \sim N(\mu,\sigma^2)
\end{equation}
geschrieben werden. In @sec-normalverteilungen zeigen wir, wie die gemeinsame 
...
1. Wie erkennt man an der gemeinsamen WMF oder WDF eines zweidimensionalen Zufallsvektors $\xi := (\xi_1,\xi_2)$, ob die
...
1. Geben Sie die Definition des Begriffs der unabhängig und identisch verteilten Zufallsvariablen wieder.

## Lösungen {.unnumbered .unlisted}

1. Siehe @def-zufallsvektor.
1. Siehe @def-multivariate-verteilung
1. Siehe @def-diskreter-zufallsvektor-und-multivariate-wahrscheinlichkeitsmassefunktion.
1. Siehe @def-kontinuierlicher-zufallsvektor-und-multivariate-wahrscheinlichkeitdichtefunktion.
1. Siehe @def-univariate-marginalverteilung.
1. Siehe @thm-marginale-wahrscheinlichkeitsmasse-und-dichtefunktionen. Man
erhält die WMF der $i$ten Komponente eines diskreten Zufallsvektors durch Summation
der Werte der WMF des Zufallsvektors über alle Komponenten des Zufallsvektors *außer* der $i$ten Komponente.
1. Siehe @thm-marginale-wahrscheinlichkeitsmasse-und-dichtefunktionen. Man
erhält die WDF der $i$ten Komponente eines kontinuierlichen Zufallsvektors durch Integration
der Werte der WDF des Zufallsvektors über alle Komponenten des Zufallsvektors *außer* der $i$ten Komponente.
1. Siehe @def-unabhaengige-zufallsvariablen.
1. Man prüft, ob sich alle Werte der gemeinsamen WMF oder WDF $p(x_1,x_2)$ 
des Zufallsvektors durch Multiplikation der entsprechenden marginalen WMF oder
WDF Werte $p(x_1)$ und $p(x_2)$ ergeben. Ist dies für alle $(x_1,x_2) \in \mathcal{X}_1 \times \mathcal{X}_2$ 
der Fall, so sind $\xi_1$ und $\xi_2$ unabhängig, ist dies nicht für alle 
$(x_1,x_2) \in \mathcal{X}_1 \times \mathcal{X}_2$, so sind $\xi_1$ und $\xi_2$ 
nicht unabhängig.
1. Siehe @def-unabhaengige-zufallsvariablen.
1. Siehe @def-unabhaengig-und-identisch-verteilte-zufallsvariablen.
