# Differenzialrechnung {#sec-differenzialrechnung}

Die Differenzialrechnung befasst sich mit der Änderung von Funktionen. Sie bildet
einerseits die Grundlage für die mathematische Modellierung dynamischer Systeme
mithilfe von *Differenzialgleichungen*, also der Beschreibung von Funktionen anhand
ihrer Änderungsraten. Zum anderen bildet die Differenzialrechnung die Grundlage
...
\frac{f(a+h)-f(a)}{h}
\end{equation}
*Newtonscher Differenzquotient*. Wie in @fig-newtonscher-differenzquotient dargestellt,
misst der Newtonsche Differenzquotient die Änderung $f(a+h)-f(a)$ von $f$ auf der $y$-Achse
pro Strecke $h$ auf der $x$-Achse. Wenn also zum Beispiel $f(a)$ und $f(a+h)$ die
Position eines Objektes zu einem Zeitpunkt $a$ und zu einem späteren Zeitpunkt
$a+h$ repräsentieren, dann ist $f(a+h)-f(a)$ die von diesem Objekt in der Zeit $h$
zurückgelegte Strecke, also seine durchschnittliche Geschwindigkeit über den Zeitraum
$h$. Für $h\to 0$ misst der Newtonsche Differenzquotient dann die instantane
Änderungsrate von $f$ in $a$, also in diesem Beispiel die Geschwindigkeit des Objektes
zu einem Zeitpunkt $a$.

![Elemente des Newtonschen Differenzquotienten.](./_figures/106-newtonscher-differenzquotient){#fig-newtonscher-differenzquotient fig-align="center" width=80%}
...
wir auch eine adaptierte Form der Leibniz-Notation und verstehen dort die
...
Zeit repräsentiert und das dann üblicherweise mit $t$ für *time* bezeichnet wird.
$\dot{f}(t)$ bezeichnet dann entsprechend die Änderungsrate von $f$ zum Zeitpunkt $t$. Die
Euler-Notation ist vor allem im Kontext multivariater reell- oder
vektorwertiger Funktionen nützlich.

Wir setzen eine Reihe von Ableitungen elementarer Funktionen als bekannt voraus,
diese sind in @tbl-ableitungen zusammengestellt. Für Beweise verweisen wir auf
die weiterführende Literatur.

\footnotesize
```{r, tbl-ableitungen, echo = F}
...
In @fig-elementare-ableitungen-1 visualisieren wir die Identitätsfunktion, eine
lineare Funktion und die Quadratfunktion zusammen mit ihrer jeweiligen Ableitung.
In Abbildung @fig-elementare-ableitungen-2 visualisieren wir die Exponential- und
Logarithmusfunktionen zusammen mit ihrer jeweiligen Ableitung.

```{r, echo = F, eval = F}
# Speichern
dev.new()
pdf(
file        = file.path("./_figures/106-elementare-ableitungen-1.pdf"),
width       = 9,
height      = 3)
...
a         = 1/2
b         = -1
fxdx      = cbind(matrix(a*x + b), matrix(a, nrow = length(x)))                  # f(x) := 0.5x - 1, f'(x) = 0.5
matplot(
x,
fxdx,
type      = "l",
...
dev.off()
```
![Ableitungen dreier elementarer Funktionen. Die Ableitung der Identitätsfunktion
ist die konstante Funktion mit Wert $1$, die Ableitung der betrachteten linear-affinen
Funktion ist die konstante Funktion mit Wert $0.5$ und die Ableitung der Quadratfunktion
ist die linear-affine Funktion mit $a = 2$ und $b = 0$](./_figures/106-elementare-ableitungen-1){#fig-elementare-ableitungen-1 fig-align="center" width=100%}

```{r, echo = F, eval = F}
# Speichern
pdf(
file        = file.path("./_figures/106-elementare-ableitungen-2.pdf"),
width       = 8,
height      = 4)
...
![Ableitungen von Exponentialfunktion und Logarithmusfunktion. Die Ableitung der
Exponentialfunktion ist die Exponentialfunktion selbst, die Ableitung der Logarithmusfunktion
hat die Funktionswerte $1/x$](./_figures/106-elementare-ableitungen-2){#fig-elementare-ableitungen-2 fig-align="center" width=85%}

Basierend auf der Definition der Ableitung einer univariaten reellwertigen Funktion
...
durch
\begin{equation}
f^{(k)} := \left(f^{(k-1)}\right)' \mbox{ für } k > 1,
\end{equation}
...
\noindent (2) Produktregel

Es sei
\begin{equation}
f:\mathbb{R} \to \mathbb{R}, x \mapsto f(x) := x^2 \sin(x).
\end{equation}
Dann hat $f$ die Form
\begin{equation}
f(x) = g_1(x)g_2(x) \mbox{ mit } g_1(x) := x^2 \mbox{ und } g_2(x) := \sin(x).
\end{equation}
Es gelten außerdem
\begin{equation}
g_1'(x) = \frac{d}{dx}\left(x^2\right) = 2x \mbox{ und } g_2'(x) = \frac{d}{dx}\left(\sin x\right) = \cos(x).
\end{equation}
Also gilt mit der Produktregel
\begin{equation}
f'(x) = g_1'(x)g_2(x) + g_1(x)g_2'(x) = 2x\sin(x) + x^2\cos(x).
\end{equation}

\noindent (3) Quotientenregel

Es sei
\begin{equation}
f:\mathbb{R} \to \mathbb{R}, x \mapsto f(x) := \frac{x^2}{x+1}.
\end{equation}
Dann hat $f$ die Form
\begin{equation}
f(x) = \frac{g_1(x)}{g_2(x)} \mbox{ mit } g_1(x) := x^2 \mbox{ und } g_2(x) := x+1.
\end{equation}
Es gelten außerdem
\begin{equation}
g_1'(x) = \frac{d}{dx}\left(x^2\right) = 2x \mbox{ und } g_2'(x) = \frac{d}{dx}\left(1 + x \right) = 1.
\end{equation}
Also gilt mit der Quotientenregel
\begin{equation}
f'(x) = \frac{g_1'(x)g_2(x) - g_1(x)g_2'(x)}{g_2^2(x)} = \frac{2x(x+1)-x^2}{(x+1)^2} = \frac{2x^2 + 2x - x^2}{(x+1)^2} = \frac{x^2 + 2x}{(x+1)^2}.
\end{equation}

\noindent (4) Kettenregel
...
Dann hat $f$ die Form
\begin{equation}
f(x) = g_1(g_2(x)) \mbox{ mit }  g_1(x) := \exp(x) \mbox{ und } g_2(x) := -x^2
\end{equation}
Es gelten außerdem
\begin{equation}
g_1'(x) = \frac{d}{dx}\left(\exp(x)\right) = \exp(x)
\mbox{ und }
g_2'(x) = \frac{d}{dx}\left(-x^2\right) = -2x.
\end{equation}
Also gilt mit der Kettenregel
\begin{equation}
f'(x) = g_1'(g_2(x))g_2'(x) = \exp\left(-x^2\right)\left(-2x\right) = -2x\exp\left(-x^2 \right).
\end{equation}
Man kann sich die resultierende Ableitung der Kettenregel also merken als "Die
Ableitung der äußeren Funktion an der Stelle der inneren Funktion mal die Ableitung
der inneren Funktion."

## Analytische Optimierung {#sec-analytische-optimierung}

Eine wichtige Anwendung der Differenzialrechnung ist das Bestimmen von
...
nicht möglich und es werden numerische Algorithmen zur Bestimmung von Extremstellen
...
Wir beginnen damit, die Begriffe der erwähnten Maxima und Minima von univariaten
reellwertigen Funktionen zu präzisieren.
...
\end{equation}
* ein *globales Maximum*, wenn gilt, dass
\begin{equation}
...
\end{equation}
bezeichnet. @fig-extrema visualisiert die verschiedenen Arten von Extremwerten
nach @def-extremstellen-und-extremwerte.
...
# Funktionsdefinition und Extremstellenbestimmung
f       = function(x) sin(x) + 0.15*x                                           # f(x) := sin(x) + 0.15x
fp      = function(x) cos(x) + 0.15                                             # f'(x)
a       = -3                                                                    # [a,b]
b       = 10                                                                    # [a,b]
x       = seq(a, b, length.out = 2000)                                          # [a,b]
Ic      = list(c(-3,0), c(1,2.5), c(4,5), c(7,9))                               # kritische Intervalle
xc      = sapply(Ic, function(iv) uniroot(fp, interval = iv)$root)              # kritische Punkte
yc      = f(xc)                                                                 # Funktionswerte der kritischen Punkte
types   = ifelse(-sin(xc) > 0, "local min", "local max")                        # f''(x) >< 0?
xs      = c(a, xc, b)                                                           # x*
ys      = f(xs)                                                                 # y*
gmin_i  = which.min(ys)                                                         # globales Minimum
gmax_i  = which.max(ys)                                                         # globales Maximum
...
ylab    = "f(x)",
ylim    = c(-1.5,2.5),
main    = TeX("$f(x) = sin(x) + 0.15x$ auf  $[-3,10]$"))
points(
xc[types=="local min"],
yc[types=="local min"],
pch     = 21,
...
legend(
"topleft",
legend  = c("f", "Lokales Minimum", "Lokales Maximum", "Globales Minimum", "Globales Maximum"),
lty     = c(1, NA, NA, NA, NA),
pch     = c(NA, 21, 24, 19, 17),
pt.bg   = c(NA, "gray", "gray", NA, NA),
lwd     =  c(2, NA, NA, NA, NA), bty = "n")
dev.off()
```

![Extremstellen einer Funktion.](./_figures/106-extrema){#fig-extrema fig-align="center" width=85%}

:::{#thm-notwendige-bedingung-für-extrema}
## Notwendige Bedingung für Extrema
$f$ sei eine univariate reellwertige Funktion, die in einer Umgebung von $x_0$ differenzierbar ist. Wenn $x_0$ eine innere Extremstelle von $f$ ist, dann gilt
\begin{equation}
x_0 \mbox{ ist innere Extremstelle von } f \Rightarrow f'(x_0) = 0.
\end{equation}
:::

Wenn $x_0$ eine innere Extremstelle von $f$ ist, dann ist also die erste Ableitung von
$f$ in $x_0$ gleich null. Anstelle eines Beweises überlegen wir uns, dass zum
Beispiel an einer lokalen Maximalstelle $x_0$ von $f$ gilt: links von $x_0$
...
\end{equation}
gilt, dann hat $f$ an der Stelle $x_0$ ein lokales Minimum.

* Wenn für $x_0 \in U \subseteq \mathbb{R}$
\begin{equation}
f'(x_0) = 0 \mbox{ und } f''(x_0) < 0
\end{equation}
gilt, dann hat $f$ an der Stelle $x_0$ ein lokales Maximum.
:::
...
dev.new()
pdf(
file        = file.path("./_figures/106-analytische-optimierung-1.pdf"),
width       = 5,
height      = 5)
library(latex2exp)
par(
...
![Analytische Optimierung von $f(x) := (x-1)^2$](./_figures/106-analytische-optimierung-1){#fig-analytische-optimierung-1 fig-align="center" width=50%}

Insbesondere die hinreichenden Bedingungen für lokale Extrema legen folgendes
*Standardverfahren* zur Bestimmung von lokalen Extremstellen nahe.
...
3. Evaluation von $f''(x^*)$: Wenn $f''(x^*) > 0$ ist, dann ist $x^*$ eine lokale
Minimalstelle von $f$; wenn $f''(x^*) < 0$ ist, dann ist $x^*$ eine lokale Maximalstelle
von $f$; wenn $f''(x^*) = 0$ ist, dann erlaubt dieses Kriterium keine Entscheidung.
:::
...
\end{equation}
Die erste Ableitung von $f$ aus @fig-analytische-optimierung-1 ergibt sich
...
x^* = 1.
\end{equation}
$x^* = 1$ ist folglich eine Minimalstelle von $f$ mit zugehörigem Minimalwert $f(1) = 0$.

## Differenzialrechnung multivariater reellwertiger Funktionen {#sec-differentialrechnung-multivariater-reellwertiger-funktionen}
...
Ein Beispiel für eine multivariate reellwertige Funktion mit $n:=2$ ist
\begin{equation}
...
pdf(
file        = "./_figures/106-bivariate-funktion.pdf",
width       = 8,
height      = 4)
library(latex2exp)
par(
...
dev.off()
```
![Visualisierungen einer bivariaten Funktion. Die linke Darstellung visualisiert
die Funktionswerte als Fläche über der zweidimensionalen Definitionsmenge der Funktion.
Auch wenn diese Darstellung einen gewissen dreidimensionalen Charakter hat, handelt
es sich um eine bivariate Funktion mit entsprechend zweidimensionaler Definitionsmenge.
Die rechte Abbildung visualisiert Linien mit gleichen Funktionswerten in der zweidimensionalen
Definitionsmenge der Funktion.](./_figures/106-bivariate-funktion){#fig-bivariate-funktion fig-align="center" width=100%}
...
Funktionen zu erweitern. Dazu führen wir zunächst die Begriffe der *partiellen
...
wenn der Grenzwert
\begin{equation}
\frac{\partial}{\partial x_i}f(a) := \lim_{h\to 0} \frac{f(a + he_i)-f(a)}{h}
\end{equation}
...
In @def-partielle-ableitung bezeichnet $e_i \in \mathbb{R}^n$ den $i$ten
kanonischen Einheitsvektor, für den gilt, dass $(e_i)_j = 1$ für $i=j$ und
$(e_i)_j = 0$ für $i \neq j$ mit $j = 1,...,n$ (vgl. @def-kanonische-basis-und-kanonische-einheitsvektoren).
In Analogie und Verallgemeinerung zum Newtonschen Differenzquotienten misst der hier auftretende Differenzquotient
\begin{equation}
\frac{f(x + he_i)-f(x)}{h}
\end{equation}
damit die Änderung $f(x+he_i)-f(x)$ von $f$ pro Strecke $h$ in Richtung $e_i$. Wir
visualisieren die Bestandteile dieses Quotienten im Falle einer bivariaten Funktion
in @fig-partieller-newtonscher-differenzquotient.

![Komponenten des partiellen Newtonschen Differenzquotienten](./_figures/106-partieller-newtonscher-differenzquotient){#fig-partieller-newtonscher-differenzquotient fig-align="center" width=60%}

Für $h\to 0$ misst der Differenzquotient entsprechend die *Änderungsrate* von $f$ in $x$ in Richtung $e_i$.
Wie bei der Betrachtung von Ableitungen gilt, dass $\frac{\partial}{\partial x_i}f(x)$ eine Zahl, $\frac{\partial}{\partial x_i}f$
...
für $i = 1,...,n$ insgesamt $n$ zweite partielle Ableitungen $\frac{\partial^2}{\partial x_j\partial x_i}f$
...
Schwarz besagt insbesondere auch, dass bei Bildung der zweiten partiellen
Ableitungen die Reihenfolge des partiellen Ableitens irrelevant ist. Der Satz
erleichtert auf diese Weise die Berechnung von zweiten partiellen Ableitungen und
hilft darüber hinaus, analytische Fehler bei der Berechnung ihrer zweiten partiellen
Ableitungen aufzudecken. Wir verdeutlichen dies in Fortführung obigen Beispiels.
...
$$
f:\mathbb{R}^{2}\to \mathbb{R}, x\mapsto f(x):=x_1^2+x_2^2
$$ {#eq-partielle-ableitung-beispiel-1}
...
Als weiteres Beispiel wollen wir die partiellen Ableitungen erster und zweiter
Ordnung der Funktion
$$
f:\mathbb{R}^{3}\to \mathbb{R}, x\mapsto f(x):=x_1^2+x_1x_2+x_2\sqrt{x_3}
$$ {#eq-partielle-ableitung-beispiel-2}
...
\end{equation}
sind. Für $n = 1$ gilt $\nabla f(x) = f'(x)$. Eine wichtige Eigenschaft des
...
Für die in @eq-partielle-ableitung-beispiel-1 betrachtete Funktion $f: \mathbb{R}^2 \to \mathbb{R}$ gilt
\begin{equation}
\nabla f(x) :=
\begin{pmatrix}
\frac{\partial}{\partial x_1} f(x) 	\\
...
In @fig-gradientbeispiel visualisieren wir farbkodiert ausgewählte Werte dieses
Gradienten für $(0.7,0.7)^T$, $(-0.3,0.1)^T$, $(-0.5,-0.4)^T$ und $(0.1,-1.0)^T$.
...
# Visualisierung
pdf(
file        = "./_figures/106-gradientbeispiel.pdf",
width       = 5,
height      = 5)
library(latex2exp)
par(
...
dev.off()
```
![Exemplarische Gradientenwerte der bivariaten Funktion $f(x) = x_1^2 + x_2^2$.](./_figures/106-gradientbeispiel){#fig-gradientbeispiel fig-align="center" width=70%}


**Beispiel (2)**

Für die in @eq-partielle-ableitung-beispiel-2 betrachtete Funktion $f: \mathbb{R}^3 \to \mathbb{R}$ gilt
\begin{equation}
\nabla f(x) :=
\begin{pmatrix}
\frac{\partial}{\partial x_1} f(x) 	\\
