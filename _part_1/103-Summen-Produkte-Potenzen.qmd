...
Für eine sinnvolle Benutzung des Summenzeichens ist es essenziell, mithilfe
...
einzeln durch $n$ geteilt werden und die entsprechenden Ergebnisse dann aufaddiert
werden. Genauer gilt durch Anwendung von @thm-rechenregeln-für-summen (3) mit $a = 1/n$, dass
\begin{equation}
\frac{1}{n}\sum_{i=1}^n x_i = \sum_{i=1}^n \frac{x_i}{n}.
\end{equation}
So ist zum Beispiel der Mittelwert von $x_1 := 1, x_2 := 4, x_3 := 2$ und $x_4 := 1$ gegeben durch
\begin{equation}
...
\begin{align}
\begin{split}
\sum_{i=1}^3 \sum_{j = 1}^2 (x_iy_j)
& = \sum_{i=1}^3 (x_iy_1 + x_iy_2)                                              \\
...
Superskript zu Lauf- und Endindex Sinn ergibt. Die genaue Bezeichnung des
...
oft vereinfachen. Wir listen einige im folgenden Theorem. Dabei machen wir vorgreifend
...
:::{#def-nte-wurzel}
## $n$-te Wurzel
Für $a \in \mathbb{R}_{\ge 0}$ und $n \in \mathbb{N}$ ist die *$n$-te Wurzel von $a$*
definiert als die nichtnegative reelle Zahl $r$ mit
\begin{equation}
r^n = a.
\end{equation}
:::
...
Es sei $a \in \mathbb{R}_{\ge 0}$, $n \in \mathbb{N}$ und $r$ die $n$-te Wurzel von $a$. Dann gilt
\begin{equation}
r = a^{\frac{1}{n}}
\end{equation}
:::
...
## Wurzel und Betrag
Es sei $x \in \mathbb{R}$ und es bezeichne
\begin{equation}
|x| :=
\begin{cases}
x  & \mbox{für } x \ge 0 \\
