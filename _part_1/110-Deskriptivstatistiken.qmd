...
an und bietet neben grafischen Darstellungen größerer Datensätze auch Maßzahlen an,
...
eine Zahl vorliegt, zusammenzufassen. Wir betrachten dabei insbesondere *Häufigkeitsverteilungen*
...
write.csv(D, "./_data/110-deskriptive-statistik.csv", row.names = FALSE)
```
...
knitr::kable(D[,1:2], digits = 2, align = "c")  # R-Markdown-Tabellenoutput
```
...
die Werte 18 und 22 jeweils 21-mal bzw. zweimal auftreten. Sowohl die absolute als auch
...
visualisiert. Folgender **R** Code nutzt zu diesem Zweck die Funktion `barplot()`.
...
natürlich der Inspektion des Dataframes `H`. Beide Häufigkeitsverteilungen
...
file          = "./_figures/110-haeufigkeitsverteilungen.pdf",  # Dateiname
width         = 7,  # Breite (inch)
height         = 4)  # Höhe (inch)
...
```
\normalsize
![Absolute und relative Häufigkeitsverteilungen der Pre-Interventions-BDI-II-Werte.](./_figures/110-haeufigkeitsverteilungen){#fig-haeufigkeitsverteilungen fig-align="center" width=80%}
...
@sec-haeufigkeitsverteilungen betrachteten Fall sinnvoll angewendet werden.
Histogramme verallgemeinern also die im vorherigen Abschnitt betrachteten
Häufigkeitsverteilungen bei Datensätzen mit mehrfach auftretenden Werten.
...
genau die Werte eines Datensatzes zu Klassen zusammengefasst werden. Der entscheidende
...
das Argument `breaks` festgelegt. Speziell ist `breaks` der **R** Vektor
...
\begin{equation}
k := \left\lceil \frac{b_k - b_0}{d} \right\rceil.
\end{equation}
...
auch kleiner sein kann. Folgender **R** Code verdeutlicht diesen Effekt.
...
\end{equation}
Folgender **R** Code implementiert diese Wahl für den vorliegenden Datensatz.
\tiny
```{r}
...
der Werte des Beispieldatensatzes anhand der oben spezifizierten Klassen.

\tiny
```{r, eval = F}
# Abbildungsparameter
pdf(  # PDF Kopiefunktion
file        = "./_figures/110-histogramme.pdf",  # Dateiname
width       = 6,  # Breite (inch)
height      = 9)  # Höhe (inch)
par(  # Abbildungsparameter
...
und Klassenbreite abhängig.](./_figures/110-histogramme){#fig-histogramme fig-align="center" width=90%}
...
\end{equation}
die *kumulative relative Häufigkeitsverteilung* der Zahlenwerte von $y$.
:::
...
```{r, eval = F}
pdf(  # PDF Kopiefunktion
file        = "./_figures/110-kumulative-haeufigkeitsverteilungen.pdf",  # Dateiname
...
![Absolute und relative kumulative Häufigkeitsverteilungen der Pre-Interventions-BDI-II-Werte.](./_figures/110-kumulative-haeufigkeitsverteilungen){#fig-kumulative-haeufigkeitsverteilungen fig-align="center" width=80%}
...
der relativen Häufigkeiten. Natürlicherweise kommen in Datensätzen nicht alle reellen
...
pdf(  # PDF Kopiefunktion
file    = "./_figures/110-ecdf.pdf",  # Dateiname
width   = 8,  # Breite (inch)
height  = 5)  # Höhe (inch)
...
![Empirische Verteilungsfunktion der PRE Werte.](./_figures/110-ecdf){#fig-ecdf fig-align="center" width=80%}

## Quantile und Boxplots {#sec-quantile-und-boxplots}

Die empirische Verteilungsfunktion gibt eine zumindest grafisch dargestellte
...
Werte des Datensatzes größer oder gleich $y_p$ sind. Das $p$-Quantil teilt den geordneten
...
Wir wollen zunächst das untere Quartil, also das $0.25$-Quantil, dieses Datensatzes
...
Als nächstes wollen wir das $0.80$-Quantil des Datensatzes bestimmen. Es ist
...
\tiny
```{r}
y   = c(8.5,1.5,75,4.5,6.0,3.0,3.0,2.5,6.0,9.0)  # Beispieldaten
n   = length(y)  # Anzahl Datenwerte
...
Wir betrachten exemplarisch die Bestimmung des $0.25$-Quantils des Pre-Interventions-
BDI-II-Beispieldatensatzes anhand von `type = 8`.
...
```
\normalsize
![Boxplot der Pre-Interventions-BDI-Werte des Beispieldatensatzes.](./_figures/110-boxplot){#fig-boxplot fig-align="center" width=80%}
...
Erläuterung der grafisch dargestellten Datensatzeigenschaften ist also immer nötig.
Schließlich sei angemerkt, dass die Darstellung von Datensätzen
...
die Anzahl der Datenwerte. Der hier definierte Mittelwert wird auch als *arithmetisches Mittel*
...
```
```{r, echo  = F}
cat("Mittelwert der PRE-BDI-II-Daten ", y_bar)  # Ausgabe
```
\normalsize
...
Datensatzes fest, die seine Einordnung als Maß der zentralen Tendenz eines Datensatzes
plausibel machen.
...
yz_bar_2     = y_bar + z_bar  # Summe der Mittelwerte der Pre- und Post-Daten
```
```{r, echo  = F}
...
Eine linear-affine Transformation eines Datensatzes transformiert den Mittelwert
...
(in @fig-median B speziell in der Mitte der Werte $y_{(3)}$ und $y_{(4)}$). Damit sind
...
![Bestimmung und Eigenschaften des Medians.](./_figures/110-median){#fig-median fig-align="center" width=80%}
...
des Medians bereit, wie folgender **R** Code demonstriert.
...
(z.B. sollte ein BDI-II-Wert von 10.000 schlicht nicht vorkommen) oder großen
...
auffallen. Es gibt allerdings ein ganzes Unterfeld der Statistik, das sich mit -
im weitesten Sinne - ausreißerunabhängigen Maßen beschäftigt, vgl. z.B. @huber1981.
...
$h : W \to \mathbb{N}$ sei die absolute Häufigkeitsverteilung der Zahlenwerte von $y$.
...
in einem Datensatz, der sich symmetrisch um einen Wert von etwa $y = 50$ verteilt
und für den Werte nah bei diesem zentralen Wert häufig
...
in einem Datensatz, in dem die Datenwerte um zwei Werte von etwa $y = 25$ und $y = 75$
gehäuft auftreten und jeweils in positive und negative Richtung von diesen Werten
entfernt eher selten auftreten. So liegen zum Beispiel um $y = 50$ recht wenige
Datenpunkte dieses Datensatzes. Mittelwert und Median nehmen nun aber gerade Werte
um $y = 50$ an. Als solche geben sie bei diesem Datensatz also keine Auskunft darüber,
welche Werte besonders häufig vorkommen, sondern lediglich darüber, wo sich der
mittlere "physikalische Schwerpunkt" des Datensatzes befindet. Der Modalwert
hingegen liegt, aufgrund des etwas häufigeren Auftretens von Werten um $y = 75$,
bei dem größeren der beiden Datenhäufungspunkte. Insgesamt betrachtet beschreibt
keines der drei Maße bei einem solchen Datensatz, den man auch *bimodal* nennt,
die zentrale Tendenz der Datenwerte wirklich zufriedenstellend.

@fig-visuelle-intuition C visualisiert die Lage von Mittelwert, Median und Modalwert
in einem Datensatz, in dem die Datenwerte in der betrachteten Breite
...
Mittelwert und Median liegen wiederum im Sinne des "physikalischen Schwerpunkts"
des Datensatzes bei etwa $y = 50$, auch wenn die Werte dieses Datensatzes dort
keinerlei Tendenz haben, häufiger als andernorts aufzutreten. Wirklich zufriedenstellend
...
verteilt ist. Die Daten dieses Datensatzes nehmen nur positive Werte an, viele
...
```{r, echo = F, eval = F}
pdf(
file   = "./_figures/110-visuelle-intuition.pdf",
width  = 12,
height = 12)
par(
family     = "sans",
...
![Visuelle Intuition zu Maßen zentraler Tendenz bei verschiedenen Datenverteilungsformen](./_figures/110-visuelle-intuition){#fig-visuelle-intuition fig-align="center" width=100%}
...
Mittelwert erschließt sich die volle Bedeutung letzterer Maße vor allem im Kontext
...
an dieser Stelle noch nicht erschließen, sondern ergibt nur vor dem Hintergrund
eines frequentistischen Inferenzmodells Sinn und wird in @sec-parameterschaetzung erläutert werden.

Wir fassen die obige Diskussion im folgenden Theorem zusammen.
...
$y = (y_1,...,y_n)$ sei ein Datensatz mit $n > 1$ und $\tilde{s}^2$ und $s^2$ seien seine
...
und $\frac{1}{n-1} > 0$ für $n > 1$ ergeben sich $\tilde{s}^2 \ge 0$ und $s^2 \ge 0$
...
:::{#thm-verschiebungssatz-der-unkorrigierten-empirischen-varianz}
...
Wir reproduzieren @thm-verschiebungssatz-der-unkorrigierten-empirischen-varianz
beispielhaft mithilfe folgenden **R** Codes.

\tiny
```{r}
y             = D$PRE  # Pre-Interventions-BDI-II-Daten
s2_tilde     = mean(y^2) - (mean(y))^2  # \bar{y^2} - \bar{y}^2
cat("Unkorrigierte empirische Varianz nach dem Verschiebungssatz : ", round(s2_tilde,3))  # Ausgabe
```
\normalsize

Man beachte, dass der Verschiebungssatz der unkorrigierten empirischen
Varianz für die empirische Varianz nicht zutrifft, da in diesem Fall der
Multiplikationsfaktor $\frac{1}{n-1}$ nicht auf die entsprechenden Mittelwerte führt.
...
Abweichungsquadrat eines Datensatzwertes von seinem Mittelwert. Ist die Einheit
des Datensatzes dabei zum Beispiel in einer Reaktionszeitaufgabe die Millisekunde,
...
s := \sqrt{s^2}
\end{equation}
und die *unkorrigierte empirische Standardabweichung* von $y$ ist
...
Version kann auch die unkorrigierte empirische Standardabweichung leicht in die
empirische Standardabweichung umgerechnet werden und umgekehrt, es gelten hier offenbar
\begin{equation}
\tilde{s} = \sqrt{\frac{n-1}{n}}s \mbox{ und } s = \sqrt{\frac{n}{n-1}}\tilde{s}.
\end{equation}

Folgender **R** Code demonstriert die Bestimmung der unkorrigierten empirischen
Standardabweichung und der empirischen Standardabweichung anhand der in
@def-empirische-standardabweichung festgelegten Formeln.
...
Die **R** Funktion `sd()` bestimmt per Default die empirische Standardabweichung.
...
```
```{r}
sz        = a*sd(y)  # Empirische Standardabweichung nach Theorem
```
```{r, echo = F}
cat("Empirische Standardabweichung nach Transformation : ", round(sz,3))  # Ausgabe
```
