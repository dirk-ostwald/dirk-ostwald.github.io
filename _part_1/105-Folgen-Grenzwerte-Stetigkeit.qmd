...
Grenzwertsatz wiederum bildet die Grundlage für die weit verbreitete
...
erlaubt also einen informierten Zugang zum Studium des Zentralen Grenzwertsatzes.
Weiterhin ermöglichen die in diesem Kapitel behandelten Themen zumindest einen
ersten Einstieg in das Verständnis der Stetigkeit und Glattheit von Funktionen,
die beispielsweise in der nichtlinearen Optimierung zur Bestimmung von Parameterschätzern
...
Man beachte, dass eine reelle Folge, weil es unendlich viele natürliche Zahlen gibt,
...
\end{equation}
:::
Nach @def-grenzwert-einer-folge kann der Grenzwert einer Folge existieren,
...
Für eine konvergente Folge, also eine Folge, deren Grenzwert existiert, gilt nach
@def-grenzwert-einer-folge, dass sich für ein beliebig kleines, aber positives
$\epsilon$ ein $m \in \mathbb{N}$ angeben lässt, sodass für alle Folgenglieder
...
xt        = matrix(rep(NaN,x_res*(n_max+1)), nrow = x_res)                      # x-terms of the summation definition
...
ylab      = " ",
xlab      = "x",
main      = TeX("$f_n(x) := \\sum_{k=0}^n \\frac{x^k}{k!}$"))
lines(
x,
exp(x),
type      = "l",
...
in $a = 1$ durch Darstellung von Folgengliedern $x_n \to 1$ als Punkte parallel zur x-Achse
und den entsprechenden Folgengliedern $f(x_n)$ auf dem Funktionsgraphen. Offenbar
...
definiert als $x_n := 1 - (1/2)^n$ für $n = 1,2,...$](./_figures/105-grenzwert-einer-funktion){#fig-grenzwert-einer-funktion fig-align="center" width=50%}
...
\end{equation}
Bei stetigen Funktionen können also Grenzwertbildung und Auswertung der Funktion
vertauscht werden.
