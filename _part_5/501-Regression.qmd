...
fname       = file.path("_data/501-regression.csv")                             # Speichern            
write.csv(D, file = fname, row.names = FALSE)

# Datensatzvisualisierung
library(latex2exp)
pdf(
file        =  "./_figures/501-beispieldatensatz.pdf",
width       = 5,
height      = 5)
par(
family      = "sans",
...
![Beispieldatensatz.](./_figures/501-beispieldatensatz){#fig-beispieldatensatz fig-align="center" width=50%}
...
$f_{\beta}\left(x_{i}\right)$ ihr Minimum annimmt, Ausgleichsgerade für den
...
library(latex2exp)
pdf(
file        = file.path("./_figures/501-ausgleichsgerade-1.pdf"),
width       = 12,
height      = 4)
par(
family      = "sans",
...
vor dem Hintergrund des Beispieldatensatzes.](./_figures/501-ausgleichsgerade-1){#fig-ausgleichsgerade-1 fig-align="center" width=100%}

Werte von $\beta_{0}$ und $\beta_{1}$ minimiert werden. Da hierbei eine Summe 
von quadrierten Abweichungen zwischen Datenpunkten und Werten der Ausgleichsgerade 
minimiert wird, spricht man auch oft etwas ungenau von der *Methode der kleinsten Quadrate* 
(engl. *method of least squares*). Abbildung 1.3 zeigt die vertikalen Abweichungen 
zwischen $y_{i}$ und $\beta_{0}+\beta_{1} x_{i}$ für $i=1, \ldots, n$ 
...
library(latex2exp)
pdf(
file        = "./_figures/501-ausgleichsgerade-2.pdf",
width       = 12,
height      = 4)
par(
family      = "sans",
...
![Vertikale Abweichungen und Quadratsummen bei unterschiedlichen Parameterwerten.](./_figures/501-ausgleichsgerade-2){#fig-ausgleichsgerade-2 fig-align="center" width=100%}
...
D           = read.csv("./_data/501-regression.csv")
...
beta_0_hat  = y_bar - beta_1_hat*x_bar                                          # \hat{\beta}_0, Offsetparameter
...
```{r, echo = FALSE, eval = FALSE}
pdf(
file        = "./_figures/501-ausgleichsgerade-3.pdf",
width       = 5,
height      = 5)
par(
family      = "sans",
...
![Ausgleichsgerade für den Beispieldatensatz.](./_figures/501-ausgleichsgerade-3){#fig-ausgleichsgerade-3 fig-align="center" width=50%}
...
\end{equation}
der quadrierten vertikalen Abweichungen der $y_{i}$ von den Funktionswerten $f_{\beta}\left(x_{i}\right)$ ihr Minimum annimmt, das *Ausgleichspolynom $k$-ten Grades für den Datensatz $\left\{\left(x_{1}, y_{1}\right), \ldots,\left(x_{n}, y_{n}\right)\right\}$*.
:::
...
Funktion $q$ ihr Minimum annimmt an späterer Stelle im Rahmen der Theorie des
...
fname       = "_data/501-regression.csv"                                        # Datendatei
...
# Visualisierung
pdf(
file        = "./_figures/501-ausgleichspolynom.pdf",
width       = 8,
height      = 8)
par(
family      = "sans",
...
![Ausgleichspolynome ersten bis vierten Grades für den Beispieldatensatz.](./_figures/501-ausgleichspolynom){#fig-ausgleichspolynom fig-align="center" width=100%}
...
Es sei
$$
y_{i}=\beta_{0}+\beta_{1} x_{i}+\varepsilon_{i} \text { mit } \varepsilon_{i} \sim N\left(0, \sigma^{2}\right) \text { u.i.v. für } i=1, \ldots, n
$$ {#eq-modell-der-einfache-linearen-regression}
wobei

* $y_{i}$ beobachtbare Zufallsvariablen sind, die Werte einer abhängigen Variable modellieren,
...
$y_{i}$ anhand der Werte $x_{i}$ einer unabhängigen Variable, der Parameterwerte 
...
Zufallsvariable $\varepsilon_{i}$ impliziert dabei eine Normalverteilung von $y_{i}$. 
...
\begin{equation}
y_{i} \sim N\left(\mu_{i}, \sigma^{2}\right) \text { u.v. für } i=1, \ldots, n
\end{equation}
schreiben.
:::

:::{.proof} 
Wir zeigen die Äquivalenz für ein $i$, die Unabhängigkeit der $y_{i}$ 
...
\begin{equation}
y_{i}=f\left(\varepsilon_{i}\right) \text { mit } f: \mathbb{R} \rightarrow \mathbb{R}, \varepsilon_{i} \mapsto f\left(\varepsilon_{i}\right):=\varepsilon_{i}+\left(\beta_{0}+\beta_{1} x_{i}\right)
\end{equation}
...
@thm-datenverteilung-der-einfachen-linearen-regression besagt insbesondere, dass die Datenvariablen $y_{i}$ univariat normalverteilte Zufallsvariablen sind, deren Erwartungswertparameter jeweils 
...
library(latex2exp)
pdf(
file        = "./_figures/501-modell-elr.pdf",
width       = 7,
height      = 5)
par(
family      = "sans",
...
for(i in 1:n){
  y[i] = rnorm(1, mu[i], sqrt(sigsqr))                                                  # y_i \sim N(\beta_0 + \beta_1x_i, \sigma^2)
}
...
c(TeX("$x_i$"),                                          
  ...
  TeX("$N(y;\\beta_0 + \\beta_1x_i,\\sigma^2)$"),
  TeX("$y_i$")),
cex         = .9,                                                               
lty         = c(NA,NA,1,NA),   
pch         = c(16,16,16,16),                                                        
...
![Modell der einfachen linearen Regression für $\beta_{0}:=0, \beta_{1}:=1$ und $\sigma^{2}:=1$.](./_figures/501-modell-elr){#fig-modell-elr fig-align="center" width=70%}
...
Maximum-Likelihood-Prinzips gewonnen werden. Insbesondere stellt sich dabei heraus,
dass die Maximum-Likelihood-Schätzer des Offset- und des Steigungsparameters mit
den Werten der Ausgleichsgeradenparameter identisch sind. Dies ist eine der 
Aussagen des folgenden Theorems. Wir verzichten hier bei den Schätzern aus Gründen
...
y_{i}=\beta_{0}+\beta_{1} x_{i}+\varepsilon_{i} \text { mit } \varepsilon_{i} \sim N\left(0, \sigma^{2}\right) \text { u.i.v. für } i=1, \ldots, n
\end{equation}
das Modell der einfachen linearen Regression. Dann sind Maximum-Likelihood-Schätzer
der Modellparameter $\beta_{0}, \beta_{1}$ und $\sigma^{2}$ gegeben durch
\begin{equation}
...
:::{.proof} 
Wir zeigen zunächst, dass die Ausgleichsgeradenparameter $\hat{\beta}_{0}$ und
$\hat{\beta}_{1}$ den entsprechenden Maximum-Likelihood-Schätzern gleichen. Dazu
halten wir zunächst fest, dass aufgrund der Unabhängigkeit der $y_{1}, \ldots,y_{n}$ 
...
```{r}
# Einlesen des Beispieldatensatzes
fname       = "./_data/501-regression.csv" 
