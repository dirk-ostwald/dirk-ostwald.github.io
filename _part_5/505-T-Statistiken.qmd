...
Konfidenzintervalle und Hypothesentests. Wir betrachten hier zunächst nur die
...
Geeignete Wahlen des Kontrastgewichtsvektors $c \in \mathbb{R}^{p}$ und des
...
dient der Kontrastgewichtsvektor dazu, den Zufallsvektor $\hat{\beta} \in \mathbb{R}^{p}$
in die Zufallsvariable $c^{T} \hat{\beta}$ zu transformieren und sichert damit
...
Der Nenner der T-Statistik stellt dabei sicher, dass insbesondere die adäquate
...
wie bei der Bestimmung von Konfidenzintervallen (vgl. @sec-konfidenzintervalle-fuer-betaparameterkomponenten) 
$\beta_{0}:=\beta$ oder gilt in einem Testszenario bei Zutreffen der Nullhypothese 
...
library(latex2exp)
pdf(
file        = "./_figures/505-t-statistik-uinv.pdf",
width       = 8,
height      = 4)
par(
family      = "sans",
...
![Verteilungen der T-Statistik bei unabhängig und identisch normalverteilten Zufallsvariablen.](./_figures/505-t-statistik-uinv){#fig-t-statistik-uinv fig-align="center" width=100%}
...
so dass die T-Statistik zur Evaluation des Steigungsparameters der einfachen linearen
...
library(latex2exp)
pdf(
file        = "./_figures/505-t-statistik-elr.pdf",
width       = 8,
height      = 4)
par(
family      = "sans",
...
![Verteilungen der T-Statistik bei einfacher linearer Regression.](./_figures/505-t-statistik-elr){#fig-t-statistik-elr fig-align="center" width=100%}
...
\begin{equation}
T 
= \frac{e_{j}^{T} \hat{\beta}-e_{j}^{T} \beta}{\sqrt{\sigma^{2} e_{j}^{T}\left(X^{T} X\right)^{-1} e_{j}}}
...
\begin{equation}
\delta 
= \frac{e_{j}^{T} \beta-e_{j}^{T} \beta}{\sqrt{\sigma^{2} e_{j}^{T}\left(X^{T} X\right)^{-1} e_{j}}}
= 0
\end{equation}
Damit gilt dann auch sofort, dass $T_{j} \sim t(n-p)$. Weiterhin erinnern
...
& =\mathbb{P}\left(\hat{\beta}_{j}-t_{\delta} \hat{\sigma} \sqrt{\lambda_{j}} \leq \beta_{j} \leq \hat{\beta}_{j}+t_{\delta} \hat{\sigma} \sqrt{\lambda_{j}}\right) \\
& =\mathbb{P}\left(\left[\hat{\beta}_{j}-\hat{\sigma} \sqrt{\lambda_{j}} t_{\delta}, \hat{\beta}_{j}+\hat{\sigma} \sqrt{\lambda_{j}} t_{\delta}\right] \ni \beta_{j}\right) \\
& =\mathbb{P}\left(\kappa_{j} \ni \beta_{j}\right)
\end{aligned}
\end{equation}
...
library(latex2exp)
pdf(
file        = "./_figures/505-elr-konfidenzintervalle.pdf",
width       = 9,
height      = 7)
...
dev.off()
```
![Konfidenzintervalle für Betaparameterkomponenten bei einfacher linearer Regression.](./_figures/505-elr-konfidenzintervalle){#fig-elr-konfidenzintervalle fig-align="center" width=100%}
