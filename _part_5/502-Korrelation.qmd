...
fname = "./_data/502-korrelation.csv"                                           # Dateipfad
...
![Mechanik der Stichprobenkorrelationssummenterme.](./_figures/502-korrelationsterme){#fig-korrelationsterme fig-align="center" width=70%}
...
library(latex2exp)
pdf(
file        = "./_figures/502-korrelationsbeispiele.pdf",
width       = 11,
height      = 11)
par(
family      = "sans",
...
![Beispiele bivariater Datensätze und ihrer Stichprobenkorrelationen.](./_figures/502-korrelationsbeispiele){#fig-korrelationsbeispiele fig-align="center" width=100%}
...
Ist die Stichprobenkorrelation zum Beispiel $r_{x y}=0.5$, dann ist
...
fname       = "./_data/502-korrelation.csv"
...
library(latex2exp)
pdf(
file        = "./_figures/502-erklaerte-werte-und-residuen.pdf",
width       = 5,
height      = 5)
par(
family      = "sans",
...
![Ausgleichsgerade, erklärte Werte und Residuen für den Beispieldatensatz..](./_figures/502-erklaerte-werte-und-residuen){#fig-erklaerte-werte-und-residuen fig-align="center" width=60%}
...
library(latex2exp)
pdf(
file        = "./_figures/502-rlinearitaet.pdf",
width       = 11,
height      = 4)
par(
family      = "sans",
...
![Korrelation und Abhängigkeit.](./_figures/502-rlinearitaet){#fig-rlinearitaet fig-align="center" width=100%}

@fig-rlinearitaet A zeigt eine Realisation des Modells
\begin{equation}
y_i=x_{i}+\varepsilon_{i} \mbox{ mit } \varepsilon_{i} \sim N(0,1) \mbox{ für } i=1,...,n.
\end{equation}
...
der $y_i$ Realisation $y_{i}$ vom Wert $x_{i}$: je höher der Wert von 
$x_{i}$, desto höher der Erwartungswert für den Wert von $y_i$. 
@fig-rlinearitaet B zeigt eine Realisation des Modells
\begin{equation}
y_i=x_{i}^{2}+\varepsilon_{i} \mbox{ mit } \varepsilon_{i} \sim N(0,1) \mbox{ für } i=1,...,n.
\end{equation}
Die Stichprobenkorrelationen des Datensatzes $\left\{\left(x_{i}, y_{i}\right)\right\}_{i=1}^{n}$ 
ergibt sich hier zu $r_{x y} = -0.01$. Die Stichprobenkorrelation ist also minimal. 
Es besteht aber auch hier eine klare Abhängigkeit des Wertes der $y_i$ 
Realisation $y_{i}$ vom Wert $x_{i}$ : je höher oder je niedriger der Wert von 
$x_{i}$, desto höher der Erwartungswert für den Wert von $y_i$, es 
...
\end{equation}
Auch hier ergibt sich das Bild einer Abhängigkeit des Wertes der $y_i$ Realisation 
...
$x$ und $y$ seien zwei Zufallsvariablen mit positiver Varianz. Dann 
besteht genau dann eine lineare-affine Abhängigkeit der Form
\begin{equation}
y=\beta_{0}+\beta_{1} x \mbox{ mit } \beta_{0}, \beta_{1} \in \mathbb{R}
\end{equation}
...
:::{.proof}
Wir beschränken uns auf den Beweis der Aussage, dass aus $y=\beta_{0}+\beta_{1} x$ 
folgt, dass $\rho(x, y)= \pm 1$ ist. Dazu halten wir zunächst fest, 
...
\end{equation}
Wegen $\mathbb{V}(x)>0$ und $\mathbb{V}(y)>0$ gilt damit $\beta_{1} \neq 0$. Es folgt dann
\begin{equation}
\beta_{1} > 0 
\Rightarrow 
...
Dabei impliziert die linear-affine Abhängigkeit von $y$ von $x$ auch 
immer die linear-affine Abhängigkeit von $x$ von $y$, denn
\begin{equation}
y = \beta_{0} + \beta_{1} x 
\Leftrightarrow
-\beta_{0} + y = \beta_{1} x 
