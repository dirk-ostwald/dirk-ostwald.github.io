...
beliebigen festen Wert $y \in \mathbb{R}^n$ von $y$ gilt also
\begin{equation}
...
$y$ die Summe der Abweichungsquadrate
\begin{equation}
(y-X\tilde{\beta})^{T}(y-X\tilde{\beta})=\sum_{i=1}^{n}\left(y_{i}-(X \tilde{\beta})_{i}\right)^{2}
\end{equation}
...
betrachten wir für einen beliebigen Wert $y \in \mathbb{R}^{n}$ von $y$ 
...
\begin{equation}
\mathbb{E}(\hat{\beta})
= \mathbb{E}\left(\left(X^{T}X\right)^{-1}X^{T}y\right) 
...
$y \in \mathbb{R}^{n}$ von $y$ konkret zu schätzen. Als Zufallsvektor ist 
...
Es sei
\begin{equation}
y = X\beta+\varepsilon \operatorname{mit} \varepsilon \sim N\left(0_{n}, \sigma^{2} I_{n}\right)
\end{equation}
das Allgemeine Lineare Modell und es sei
\begin{equation}
\hat{\beta}:=\left(X^{T}X\right)^{-1}X^{T}y .
...
Zufallsvektors
\begin{equation}
\hat{\varepsilon}_{i}:=y_{i}-\hat{y}_{i}=y_{i}-(X\hat{\beta})_{i}
\end{equation}
die *Residuen*.
:::
...
und einer Realisierung $y \in \mathbb{R}^n$ von $y$ zu schätzen. 
...
\begin{equation}
y_{i} \sim N\left(\mu, \sigma^{2}\right) \mbox{ für } i=1, \ldots, n.
\end{equation}
Schreibt man dieses Modell in seiner Designmatrixform (vgl. @eq-uinv-zven) dann gilt, 
wie unten gezeigt,
$$
\hat{\beta} = 
\frac{1}{n} \sum_{i=1}^{n} y_{i} =: \bar{y} 
\mbox{ und }
\hat{\sigma}^{2} = \frac{1}{n-1} \sum_{i=1}^{n}\left(y_{i}-\bar{y}\right)^{2} =: s_{y}^{2}
$$ {#eq-uinv-schaetzung}
In diesem Fall ist also der Betaparameterschätzer mit dem Stichprobenmittel 
$\bar{y}$ der $y_{1}, \ldots, y_{n}$ und der Varianzparameterschätzer
mit der Stichprobenvarianz $s_{y}^{2}$ der $y_{1}, \ldots, y_{n}$ identisch.
...
\begin{aligned}
\hat{\sigma}^{2} 
& = \frac{1}{n-1}(y-X\hat{\beta})^{T}(y-X\hat{\beta}) \\
& = \frac{1}{n-1}\left(v-1_{n} \bar{y}\right)^{T}\left(v-1_{n} \bar{y}\right) \\
& = \frac{1}{n-1}
\left(
\begin{pmatrix}
y_{1} \\
...
& =
\frac{1}{n-1}
\begin{pmatrix} y_{1} - \bar{y} & \cdots & y_{n}-\bar{y}\end{pmatrix}
\begin{pmatrix}
y_{1}-\bar{y}      \\
\vdots                          \\
y_{n}-\bar{y}
...
\end{pmatrix}
=\begin{pmatrix}
\bar{y}-\frac{c_{x v}}{s_{x}^{2}} \bar{x} \\
\frac{c_{x v}}{s_{x}^{2}}
\end{pmatrix} 
\mbox{ und } 
\hat{\sigma}^{2} = \frac{1}{n-2}\sum_{i=1}^{n}\left(y_{i}-\left(\hat{\beta}_{0}+\hat{\beta}_{1} x_{i}\right)\right)^{2}
$${#eq-betahat-elr}

wobei $\bar{x}$ und $\bar{y}$ die Stichprobenmittel der $x_{1}, \ldots, x_{n}$ 
und $y_{1}, \ldots, y_{n}, c_{x v}$ die Stichprobenkovarianz der $x_{1}, 
\ldots, x_{n}$ und $y_{1}, \ldots, y_{n}$ und $s_{x}^{2}$ die 
...
$y \in \mathbb{R}^n$ von $y$ den Wert
\begin{equation}
q(\tilde{\beta})
...
& =\sum_{i=1}^{n}\left(x_{i} y_{i}-x_{i} \bar{y}-\bar{x} y_{i}+\bar{x} \bar{y}\right) \\
...
& =\sum_{i=1}^{n} x_{i} y_{i}-\bar{y} n \bar{x}-\bar{x} n \bar{y}+n \bar{x} \bar{y} \\
& =\sum_{i=1}^{n} x_{i} y_{i}-n \bar{x} \bar{y}-n \bar{x} \bar{y}+n \bar{x} \bar{y} \\
...
\sum_{i=1}^{n} x_{i} y_{i}
\end{pmatrix} & =\begin{pmatrix}
\left(\frac{1}{n}+\frac{\bar{x}^{2}}{s_{x}^{2}}\right) n \bar{y}-\frac{\bar{x} \sum_{i=1}^{n} x_{i} y_{i}}{s_{x}^{2}} \\
\frac{\sum_{i=1}^{n} x_{i} y_{i}}{s_{x}^{2}}-\frac{n \bar{x} \bar{y}}{s_{x}^{2}}
\end{pmatrix} \\
& =\begin{pmatrix}
\frac{n \bar{y}}{n}+\frac{\bar{x}^{2} n \bar{y}}{s_{x}^{2}}-\frac{\bar{x} \sum_{i=1}^{n} x_{i} y_{i}}{s_{x}^{2}} \\
\frac{\sum_{i=1}^{n} x_{i} y_{i}-n \bar{x} \bar{y}}{s_{x}^{2}}
\end{pmatrix} \\
& =\begin{pmatrix}
\bar{y}+\frac{\bar{x} n \bar{x} \bar{y}-\bar{x} \sum_{i=1}^{n} x_{i} y_{i}}{s_{x}^{2}} \\
\frac{\sum_{i=1}^{n} x_{i} y_{i}-n \bar{x} \bar{y}}{s_{x}^{2}}
\end{pmatrix} \\
& =\begin{pmatrix}
\bar{y}-\frac{\sum_{i=1}^{n} x_{i} y_{i}-n \bar{x} \bar{y}}{s_{x}^{2}} \bar{x} \\
\frac{\sum_{i=1}^{n} x_{i} y_{i}-n \bar{x} \bar{y}}{s_{x}^{2}}
\end{pmatrix} \\
& =\begin{pmatrix}
\bar{y}-\frac{c_{x v}}{s_{x}^{2}} \bar{x} \\
...
\begin{equation}
y \sim N\left(X \beta, \sigma^{2} I_{n}\right) \mbox{ mit } X:=1_{n} \in \mathbb{R}^{n \times 1}, \beta:=\mu \in \mathbb{R} \mbox{ und } \sigma^{2}>0 
\end{equation}
das ALM Szenario unabhängiger und identisch normalverteilter Zufallsvariablen bei bekannter Varianz. Wir haben bereits gesehen, dass in diesem Fall $\hat{\beta}$ mit dem Stichprobenmittel $\bar{y}$ identisch ist. @thm-frequentistische-verteilung-des-betaparameterschaetzers impliziert dann mit
\begin{equation}
...
Es sei
\begin{equation}
y \sim N\left(X \beta, \sigma^{2} I_{n}\right) \mbox{ mit }\begin{pmatrix}
1 & x_{1} \\
\vdots & \vdots \\
1 & x_{n}
...
Mittelwert der Werte der unabhängigen Variable ab. Folgender **R** Code simuliert
...
library(latex2exp)
pdf(
file        = "./_figures/504-elr-beta-hat.pdf",
width       = 8,
height      = 4)
par(
family      = "sans",
...
![Frequentistische Betaparameterschätzerverteilung bei einfacher linearer Regression.](./_figures/504-elr-beta-hat){#fig-elr-beta-hat fig-align="center" width=100%}
...
im ALM im Sinne der Restricted-Maximum-Likelihood-Methode erscheint zunächst in
