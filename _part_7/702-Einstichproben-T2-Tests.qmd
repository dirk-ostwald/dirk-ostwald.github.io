...
## Anwendungsszenario {#sec-t2-anwendungsszenario}
...
Herangehensweisen an Testumfangskontrolle und Stichprobengrößenoptimierung.
...
Zufallsvektoren  $y_i \sim N(\mu,\Sigma)$ sind. Wir nehmen weiterhin an, dass 
...
library(knitr)
D           = read.csv("./_data/702-einstichproben-t2-tests.csv")               # Einlesen des Datensatzes
...
Stichprobenmittels mit $\bar{y} = (26.3, 3.0)^T$ etwas geringer aus, allerdings
...
# Datendeskription
D      = read.csv("_data/702-einstichproben-t2-tests.csv")                      # Daten einlesen
...
library(latex2exp)
pdf(
file   = "_figures/702-einstichproben-t2-deskription.pdf",
width  = 4.5,
height = 4.5)
...
dargestellt](./_figures/702-einstichproben-t2-deskription){#fig-einstichproben-t2-deskription fig-align="center" width=60%}
...
Für $i = 1,...,n$ seien $y_i$ $m$-dimensionale Zufallsvektoren, die die
...
\begin{equation}
y_i \sim N(\mu, \Sigma) \mbox{ u.i.v. für } i = 1,...,n 
\mbox{ mit } \mu \in \mathbb{R}^m, \Sigma \in \mathbb{R}^{m \times m} \mbox{ pd}.
\end{equation}
:::
...
## Modellevaluation {#sec-t2-modellevaluation}
...
definiert als
\begin{equation}
T^2 := n(\bar{y} - \mu_0)^T C^{-1}(\bar{y} - \mu_0),
\end{equation}
wobei $\bar{y}$ und $C$ das Stichprobenmittel und die Stichprobenkovarianzmatrix
der $y_1,...,y_n$ bezeichnen.
:::
Die Einstichproben-T$^2$-Teststatistik ist offenbar die mit dem Stichprobenumfang $n$ 
skalierte Mahalanobis-Distanz von $\bar{y}$ und $\mu_0$ hinsichtlich $C$ (vgl. @sec-multivariate-deskriptivstatistiken-inhalt). Damit gilt entsprechend, dass bei konstanter 
Stichprobenkovarianzmatrix die Einstichproben-T$^2$-Teststatistik $T^2$
größere Werte für eine größere euklidische Distanz von $\bar{y}$ und $\mu_0$ annimmt und
bei konstanter euklidischer Distanz von $\bar{y}$ und $\mu_0$ der Wert der
...
Es seien $y_1,...,y_n \sim N(\mu,\Sigma)$ mit $\mu \in \mathbb{R}^m$ und $\Sigma \in \mathbb{R}^{m\times m} \mbox{pd}$,
$$
\nu:= \frac{n-m}{(n-1)m}
$$ {#eq-nu}
...
library(latex2exp)
pdf(
file   = "_figures/702-f-zufallsvariable.pdf",
width  = 6,
height = 4)
...
![Exemplarische WDFen einer $f$-Zufallsvariable](./_figures/702-f-zufallsvariable){#fig-f-zufallsvariable fig-align="center" width=80%}
...
library(latex2exp)
pdf(
file       = "_figures/702-nichtzentrale-f-zufallsvariable.pdf",
width      = 6,
height     = 4)
par(
family     = "sans",
...
dev.off()
```
![Exemplarische WDFen einer nichtzentralen $f$-Zufallsvariable](./_figures/702-nichtzentrale-f-zufallsvariable){#fig-nichtzentrale-f-zufallsvariable fig-align="center" width=80%}
...
\begin{equation}
T^2 = n\frac{(\bar{y} - \mu_0)^2}{S^2} = \left(\sqrt{n}\frac{\bar{y} - \mu_0}{S} \right)^2.
\end{equation}
Dies ist offenbar das Quadrat der bekannten univariaten Einstichproben-T-Teststatistik.
Damit ist nach @thm-verteilung-der-skalierten-einstichproben-t2-teststatistik
das Quadrat der univariaten Einstichproben-T-Teststatistik nach $f(\delta,1,n-1)$
...
:::{#thm-wdf-und-kvf-der-einstichproben-t2-teststatistik}
## WDF und KVF der Einstichproben-T$^2$-Teststatistik
...
$y = f(\xi)$ mit  $f(\xi) := a\xi + b$  für  $a\neq 0$ eine WDF von $y$ definiert ist durch
$p_y(y) := (1/|a|)p_\xi((y-b)/a)$. Im vorliegenden Fall ist $\xi = \nu T^2$ mit WDF
$f(\delta,m,n-m)$ und $y := T^2 = \frac{1}{\nu}\nu T^2$, also $a = 1/\nu$ und $b = 0$.
...
Wir halten fest, dass die skalierte Einstichproben-T$^2$-Teststatistik $\nu T^2$ nach $f(\delta,m,n-m)$
nichtzentral $f$-verteilt ist, die WDF der Einstichproben-T$^2$-Teststatistik $T^2$ selbst dagegen
durch $\nu f(\nu t^2;\delta, m,n-m)$ gegeben ist. Wir simulieren diese Verteilung
...
library(latex2exp)
pdf(
file        = "_figures/702-einstichproben-t2-teststatistik.pdf",
width       = 6,
height      = 6)
...
nu          = (n-m)/(m*(n-1))
delta       = n*t(mu - mu_0) %*% solve(Sigma) %*% (mu - mu_0)
...
# Unnormalisierte T2-Teststatistik Sample
t2_min      = 0
t2_max      = 50
t2_res      = 1e3
t2          = seq(t2_min,t2_max,len = t2_res)
...
Realisierungen der Einstichproben-T$^2$-Teststatistik (grau) und ihre analytische Form (orange).](./_figures/702-einstichproben-t2-teststatistik){#fig-einstichproben-t2-teststatistik fig-align="center" width=100%}
...
\begin{equation}
\phi(y) := 1_{\{T^2 > k\}} := \begin{cases} 1 & T^2  > k \\ 0 & T^2 \le k \end{cases}.
\end{equation}
:::

In @def-einstichproben-t2-test repräsentiert wie üblich $\phi(y) = 1$ den Vorgang
des Ablehnens von $H_0$ und $\phi(y) = 0$ den Vorgang des Nichtablehnens von $H_0$. 


###  Analyse der Testgütefunktion {-}

Um für den in @def-einstichproben-t2-test definierten Test Prozeduren zur Testumfangskontrolle
...
\begin{equation}
\frac{n-m}{m(n-1)}T^2 \sim f(\delta_\mu,m,n-m) \mbox{ mit } \delta_\mu := n(\mu -\mu_0)^T\Sigma^{-1}(\mu-\mu_0)
\end{equation}
...
library(latex2exp)
pdf(
file        = "_figures/702-einstichproben-t2-gütefunktion-1.pdf",
width       = 12,
height      = 4)
par(
family      = "sans",
...
\mu_0 := (1,1)^T, 
\Sigma_1 :=   \begin{pmatrix} 1.0 & 0.0  \\ 0.0 & 1.0 \end{pmatrix}
\end{equation}](./_figures/702-einstichproben-t2-gütefunktion-1){#fig-einstichproben-t2-gütefunktion-1 fig-align="center" width=100%}
...
library(latex2exp)
pdf(
file        = "_figures/702-einstichproben-t2-gütefunktion-2.pdf",
width       = 12,
height      = 4)
par(
family      = "sans",
...
\end{equation}](./_figures/702-einstichproben-t2-gütefunktion-2){#fig-einstichproben-t2-gütefunktion-2 fig-align="center" width=100%}
  
### Testumfangskontrolle {-}

Bekanntlich erlaubt die Testumfangskontrolle die Begrenzung der größtmöglichen
...
## Testumfangskontrolle des Einstichproben-T$^2$-Tests
...
library(latex2exp)
pdf(
file        = "_figures/702-einstichproben-t2-testumfangkontrolle.pdf",
width       = 8,
height      = 4)
par(
family      = "sans",
...
dev.off()
```
![Testumfangskontrolle durch Selektion eines $\alpha_0$-abhängigen kritischen Wertes
für den Einstichproben-T$^2$-Test anhand von KVF und WDF der Einstichproben-T$^2$-Teststatistik.](./_figures/702-einstichproben-t2-testumfangkontrolle){#fig-einstichproben-t2-testumfangkontrolle fig-align="center" width=100%}
...
@fig-einstichproben-t2-testumfangkontrolle. Untenstehender **R** Code simuliert die Testumfangskontrolle für ein Einstichproben-T$^2$-Test-Szenario mit
\begin{equation}
...
# Simulation der Testumfangskontrolle
library(MASS)                                                                   # R Paket für multivariate Normalverteilungen
...
for(s in 1:nsim){                                                               # Simulationsiterationen
    ...
    Y_bar  = (1/n)*(Y %*% j_n)                                                  # Stichprobenmittel
    C      = (1/(n-1))*(Y %*% (I_n-(1/n)*J_n) %*% t(Y))                         # Stichprobenkovarianzmatrix
    T2     = n*t(Y_bar - mu_0) %*% solve(C) %*% (Y_bar - mu_0)                  # Einstichproben-T$^2$-Teststatistik
    if(T2 > k_alpha_0){                                                         # Test 1_{T^2 >= k_alpha_0}
        phi[s] = 1                                                              # Ablehnen von H_0
    } else {
        phi[s] = 0}}                                                            # Nichtablehnen von H_0
```
```{r,echo = FALSE}
...
Zufallsvektoren $y_1,...,y_n \sim N(\mu,\Sigma)$ mit unbekannten Parametern 
...
und $n=15$ bei Wahl von $\alpha_0 := 0.05$, dass $k_{0.05}=\nu^{-1}F^{-1}(1 - 0.05;2,13) \approx 8.2$ ist. Anhand von $m, n, \mu_0$, dem Stichprobenmittel $\bar{y}$ und der 
...
andernfalls nicht. Die oben entwickelte Theorie zur Testumfangskontrolle des Einstichproben-T$^2$-Test
...
library(latex2exp)
pdf(
file        = "_figures/702-einstichproben-t2-pi.pdf",
width       = 9,
height      = 9)
par(
family      = "sans",
...
einer geringeren Wahrscheinlichkeit für die Ablehnung der Nullhypothese über den
...
abzulehnen.](./_figures/702-einstichproben-t2-pi){#fig-einstichproben-t2-pi fig-align="center" width=100%}
...
library(latex2exp)
pdf(
file        = "_figures/702-einstichproben-t2-stichprobenumfang.pdf",
width       = 6,
height      = 5)
par(
family      = "sans",
...
dargestellten Funktion.](./_figures/702-einstichproben-t2-stichprobenumfang){#fig-einstichproben-t2-stichprobenumfang fig-align="center" width=70%}
...
```{r}
# Datenbereitstellung
D         = read.csv("./_data/702-einstichproben-t2-tests.csv")                 # Datensatzeinlesen 
Y         = rbind(D$y_1i, D$y_2i)                                               # Datenmatrix
...
und man lehnt die Nullhypothese ab. Der korrespondierende p-Wert ist durch 0.049 gegeben.
