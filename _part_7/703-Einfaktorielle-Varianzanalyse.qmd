# Einfaktorielle Varianzanalyse {#sec-manova-einfaktorielle-varianzanalyse}

## Anwendungsszenario {#sec-manova-anwendungsszenario}
...
fname    = "./_data/703-einfaktorielle-varianzanalyse.csv"                      # Dateiname
...
for (i in 1:p){                                                                 # Gruppeniterationen
    y_bar_i[,i] = (1/k)*(Y[,,i] %*% j_k)                                        # Stichprobenmittel \bar{y}_i
...
library(latex2exp)
pdf(
file        = "_figures/703-anwendungsbeispiel.pdf",
width       = 6,
height      = 6)
par(
family      = "sans",
...
der jeweiligen Gruppe dargestellt.](./_figures/703-anwendungsbeispiel){#fig-anwendungsbeispiel fig-align="center" width=50%}
...
Für $i = 1,...,p$ und $j = 1,...,n_i$ seien $y_{ij}$ $m$-dimensionale
...
\begin{equation}
y_{ij} \sim N(\mu_i,\Sigma) \mbox{ u.v.  mit }  \mu_i \in \mathbb{R}^m  \mbox{ und } \Sigma \in \mathbb{R}^{m \times m} \mbox{ pd}.
\end{equation}
:::

In @def-modell-der-einfaktoriellen-varianzanalyse bezeichnet $n_i$ die Anzahl
der Zufallsvektoren $y_{ij}$ der $i$ten von $p$ Gruppen experimenteller Einheiten. 
...
\begin{equation}
\hat{\Sigma} := \frac{1}{n-p}\sum_{i = 1}^p \sum_{j = 1}^{n_i} \left(y_{ij} - \hat{\mu}_i\right)\left(y_{ij} - \hat{\mu}_i\right)^T
\end{equation}
...
library(latex2exp)
pdf(
file        = "_figures/703-parameterschätzer.pdf",
width       = 6,
height      = 6)
par(
family      = "sans",
...
und 100 Realisierungen der entsprechenden multivariat normalverteilten Zufallsvektoren](./_figures/703-parameterschätzer){#fig-parameterschätzer fig-align="center" width=50%}  
...
\normalsize
## Modellevaluation {#sec-manova-modellevaluation}
...
Schließlich seien
\begin{tabular}{ll}
$T := \sum_{i = 1}^p \sum_{j = 1}^{n_i} \left(y_{ij} - \bar{y}\right)\left(y_{ij} - \bar{y}\right)^T$
&
die \textit{Totale Sum-of-Squares-Matrix}
\\
\\
$B := \sum_{i = 1}^p n_i \left(\bar{y}_{i} - \bar{y}\right)\left(\bar{y}_{i} - \bar{y}\right)^T$
&
die \textit{Between-Group Sum-of-Squares-Matrix}
\\
\\
$W := \sum_{i = 1}^p \sum_{j = 1}^{n_i} \left(y_{ij} - \bar{y}_i\right)\left(y_{ij} - \bar{y}_i\right)^T$
&
...
\begin{split}
T
& = \sum_{i=1}^p \sum_{j=1}^{n_i}(y_{ij}-\bar{y})(y_{ij}-\bar{y})^T
\\
& = \sum_{i=1}^p \sum_{j=1}^{n_i} (y_{ij}-\bar{y}_i+\bar{y}_i-\bar{y})(y_{ij}-\bar{y}_i+\bar{y}_i-\bar{y})^T
\\
& = \sum_{i=1}^p \sum_{j=1}^{n_i} \left((y_{ij}-\bar{y}_i)+(\bar{y}_i-\bar{y})\right)\left((y_{ij}-\bar{y}_i)+(\bar{y}_i-\bar{y})\right)^T
\\
& = \sum_{i=1}^p \sum_{j=1}^{n_i} \left(
                                      (y_{ij}-\bar{y}_i)(y_{ij}-\bar{y}_i)^T
								    +2(y_{ij}-\bar{y}_i)(\bar{y}_i-\bar{y})^T
				    		        + (\bar{y}_i-\bar{y})(\bar{y}_i-\bar{y})^T
				  		        \right)
\\
& = \sum_{i=1}^p \left(
                       \sum_{j=1}^{n_i} (y_{ij}-\bar{y}_i)(y_{ij}-\bar{y}_i)^T
   				      +\sum_{j=1}^{n_i}2(y_{ij}-\bar{y}_i)(\bar{y}_i-\bar{y})^T
				      +\sum_{j=1}^{n_i}(\bar{y}_i-\bar{y})(\bar{y}_i-\bar{y})^T
				 \right)
\\
& = \sum_{i=1}^p \left(
                       \sum_{j=1}^{n_i} (y_{ij}-\bar{y}_i)(y_{ij}-\bar{y}_i)^T
   				      +2\left(\sum_{j=1}^{n_i}(y_{ij}-\bar{y}_i)\right)(\bar{y}_i-\bar{y})^T
				      +n_i(\bar{y}_i-\bar{y})(\bar{y}_i-\bar{y})^T
				 \right)
\\
& = \sum_{i=1}^p \left(
                       \sum_{j=1}^{n_i} (y_{ij}-\bar{y}_i)(y_{ij}-\bar{y}_i)^T
   				      +2\left(\sum_{j=1}^{n_i}\left(y_{ij}-\frac{1}{n_i}\sum_{j=1}^{n_i} y_{ij}\right)\right)(\bar{y}_i-\bar{y})^T
				      +n_i(\bar{y}_i-\bar{y})(\bar{y}_i-\bar{y})^T
				 \right)
\\
& = \sum_{i=1}^p \left(
                       \sum_{j=1}^{n_i} (y_{ij}-\bar{y}_i)(y_{ij}-\bar{y}_i)^T
   				      +2\left(\sum_{j=1}^{n_i} y_{ij}-\sum_{j=1}^{n_i} y_{ij}\right) (\bar{y}_i-\bar{y})^T
				      +n_i(\bar{y}_i-\bar{y})(\bar{y}_i-\bar{y})^T
				 \right)
\\
& = \sum_{i=1}^p \left(
				 ...
				 \right)
\\
& = \sum_{i=1}^p n_i(\bar{y}_i-\bar{y})(\bar{y}_i-\bar{y})^T
   +\sum_{i=1}^p \sum_{j=1}^{n_i} (y_{ij}-\bar{y}_i)(y_{ij}-\bar{y}_i)^T
\\
& = B + W.
\end{split}
\end{align}
...
wurde eine Reihe von Teststatistiken für die einfaktorielle multivariate Varianzanalyse
...
sind die frequentistischen Verteilungen der Wilks'-$\Lambda$-Statistik bei
Zutreffen der Nullhypothese nur für bestimmte Anwendungsszenarien, insbesondere 
bei kleinen Werten der Datendimension $m$ und der Gruppenanzahl $p$, analytisch
...
$\bar{y}_i$ nimmt $|B+W|$ gegenüber $|W|$ zu und $\Lambda$ somit ab. Ohne Beweis 
...
Grundlage für $F$-Teststatistik im univariaten Fall bildet. Es gilt folgendes Theorem.

:::{#thm-eigenwertform-von-wilks-lambda}
## Eigenwertform der Wilks'-$\Lambda$-Statistik
...
# figure parameters
pdf(
file        = "_figures/703-spezielle-lambda-verteilungen.pdf",
width       = 6,
height      = 6)
par(
family      = "sans",
...
![Simulation spezieller Verteilungen der Wilks'-$\Lambda$-Statistik-Transformationen bei Zutreffen der Nullhypothese.](./_figures/703-spezielle-lambda-verteilungen){#fig-spezielle-lambda-verteilungen fig-align="center" width=70%}
...
library(latex2exp)
pdf(
file        = "_figures/703-tau-lambda.pdf",
width       = 6,
height      = 4)
par(
family      = "sans",
...
![Beziehung der Wilks'-$\Lambda$-Statistik und der von @rao1951 betrachteten $\tau$-Statistik in exemplarischen Anwendungsszenarien.](./_figures/703-tau-lambda){#fig-tau-lambda fig-align="center" width=70%}
...
library(latex2exp)
pdf(
file        = "_figures/703-approximative-lambda-verteilungen.pdf",
width       = 6,
height      = 6)
par(
family      = "sans",
...
![Simulation approximativer Verteilungen der Wilks'-$\Lambda$-Statistik-Transformationen bei Zutreffen der Nullhypothese.](./_figures/703-approximative-lambda-verteilungen){#fig-approximative-lambda-verteilungen fig-align="center" width=80%}
...
## Wilks'-$\Lambda$-basierter Test, Testumfangskontrolle, p-Wert
Gegeben seien Modell der einfaktoriellen multivariaten Varianzanalyse und die 
transformierte Wilks'-$\Lambda$-Statistik $\tau$ mit Verteilungsparametern
$\nu_1,\nu_2$ wie oben definiert. Weiterhin sei für $Y = (y_1,...,y_n)$ 
...
\end{equation}
ist und der p-Wert einer realisierten $\tau$-Teststatistik $\tilde{\tau}$ ergibt
sich zu
\begin{equation}
...
implizite Wahl eines kritischen Wertes zur Testumfangskontrolle im Anwendungsszenario
...
library(latex2exp)
pdf(
file        = "_figures/703-kritischer-wert-lambda.pdf",
width       = 8,
height      = 4)
par(
family      = "sans",
...
![Testumfangskontrolle durch Selektion eines $\alpha_0$-abhängigen kritischen Wertes für den Wilks'$\Lambda$-Statistik-basierten Test anhand von KVF und WDF der transformierten Wilks'$\Lambda$-Statistik $\tau$.](./_figures/703-kritischer-wert-lambda){#fig-kritischer-wert-lambda fig-align="center" width=80%}
...
$y_{ij} \sim N(\mu_i,\Sigma)$ mit unbekannten, gruppenspezifischen 
...
Wilks'-$\Lambda$-Statistik und den resultierenden, $m,p,n$-abhängigen realisierten
Wert der transformierten Wilks'-$\Lambda$-Statistik $\tau$. Wenn der berechnete Wert von
$\tau$ größer als $k_{\alpha_0}$ ist, lehnt man die Nullhypothese ab, andernfalls nicht.
Die oben entwickelte Theorie zur Testumfangskontrolle bei der einfaktoriellen multivariaten
