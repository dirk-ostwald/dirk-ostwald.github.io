...
In diesem Kapitel gehen wir immer im Sinne der in @sec-frequentistische-inferenz
...
@def-bernoullimodell durch das entsprechende Stichprobenmittel gegeben ist und in 
...
als Bezeichungen für die Varianz der Zufallsvariable $\hat{\tau}(y)$ und die 
Standardabweichung der Zufallsvariable $\hat{\tau}(y)$ zwei Maße für die 
...
$y$ sei eine Stichprobe eines Frequentischen Inferenzmodells und $\hat{\tau}$ 
...
Werte des Erwartungswerts, der Varianz und der Standardabweichung der $y_i$ mit $1\le i \le 12$. 
...
10000 Realisierungen von $y_1,...,y_{12}$ entsprechen die Stichprobenmittel 
von $\bar{y}$ und $S^2$  gemäß ihrer Erwartungstreue ihren wahren, aber unbekannten, 
Werten. Die Stichprobenstandardabweichung dagegen zeigt gemäß ihrer Verzerrtheit 
auch bei weiter ansteigenden Anzahlen von der Realsierungen von $y_1,...,y_{12}$  
...
Es sei $y = (y_1,...,y_n)$ die Stichproben eines Bernoullimodells und
...
vierte Gleichung mit der Varianz $\mathbb{V}_\mu(y_1) = \mathbb{V}_\mu(y_i) = \mu(1-\mu)$ der Stichprobenvariablen folgt.  
:::
...
mit folgender Bedeutung: ein Schätzer $\hat{\tau}_n$ für $\tau$ heißt *asymptotisch effizient*, 
wenn für große Stichprobenumfänge $n \to \infty$ die Verteilung von $\hat{\tau}_n$ 
durch eine Normalverteilung mit Erwartungswertparameter $\tau(\theta)$ und Varianzparameter 
