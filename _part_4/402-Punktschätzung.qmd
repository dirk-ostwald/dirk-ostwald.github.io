...
In diesem Kapitel gehen wir im Sinne der in @sec-frequentistische-inferenz
...
oder WDF $p_\theta$ für alle $\theta \in \Theta$ aus. $y := (y_1,...,y_n)$
...
\end{equation}
Wesen und Ziel der hier behandelten *Punktschätzung* ist es, basierend auf der
...
Die zu schätzenden Kennzahlen von $\mathbb{P}_\theta$ sind oft schlicht die wahren,
aber unbekannten, Parameter selbst. Wir widmen uns diesem Fall ausführlich in
@sec-maximum-likelihood-schaetzung. Allerdings sind viele grundlegende Resultate
der Frequentistischen Punktschätzung auch dann valide, wenn es sich bei zu schätzenden
Kennzahlen nicht um die Parameter selbst, sondern Funktionen von ihnen handelt.
Beispiel hierfür sind die Schätzung des Erwartungswerts, der Varianz oder der
Standardabweichung von $\mathbb{P}_\theta$. Beginnen wollen wir allerdings mit
der *Parameterschätzung*. Um den wahren, aber unbekannten, Parameter eines
Inferenzmodells zu schätzen, nutzt man in der Frequentistischen Inferenz sogenannte
...
Im Sinne von @def-schätzer sind Parameterpunktschätzer Schätzer mit $\tau := \mbox{id}_\Theta$.
Parameterpunktschätzer sind also Funktionen von Daten und nehmen Zahlwerte
im Parameterraum an. Als Funktionen von Zufallsvariablen sind Parameterschätzer
...
zunächst ein allgemeines Prinzip diskutieren, das es erlaubt, für ein gegebenes
Frequentistisches Inferenzmodell Parameterschätzer zu bestimmen, die, wie wir an späterer
Stelle sehen werden, garantiert bestimmte wünschenswerte Eigenschaften haben
(@sec-eigenschaften-von-maximum-likelihood-schaetzern). Dabei beziehen sich diese
Eigenschaften allgemein auf sein qualitatives Verteilungsverhalten bei
festem Stichprobenumfang bzw. im Grenzübergang zu einem unendlich großen Stichprobenumfang.
Wir führen diese Eigenschaften in @sec-schaetzereigenschaften-bei-endlichen-stichproben
und @sec-asymptotische-schaetzereigenschaften ein.

## Maximum-Likelihood-Schätzung {#sec-maximum-likelihood-schaetzung}
...
Für diese Extremalstellen stellt die Differenzialrechnung bekanntlich notwendige
...
Funktion bieten sich dann Methoden der analytischen oder der numerischen Optimierung an.
In den folgenden klassischen Beispielen nutzen wir einen analytischen Ansatz. Wir
...
(3) Auflösen nach potenziellen Maximumstellen.
...
@exm-bernoullimodell durch das entsprechende Stichprobenmittel gegeben ist und in
@thm-maximum-likelihood-schaetzer-des-normalverteilungsmodells zeigen wir, dass die
Maximum-Likelihood-Schätzer für den Erwartungswert- und Varianzparameter des
Normalverteilungsmodells aus @exm-normalverteilungsmodell durch das Stichprobenmittel
...
$\mathcal{M}$ sei das Bernoullimodell, es gelte also $y_1,...,y_n \sim \mbox{Bern}(\mu)$.
Dann ist
\begin{equation}
...
$\mathcal{M}$ sei das Normalverteilungsmodell, es gelte also
$y_1,...,y_n \sim N\left(\mu,\sigma^2\right)$. Dann sind
\begin{equation}
\hat{\mu}^{\mbox{\tiny ML}} :
...
\hat{\mu}^{\mbox{\tiny ML}} = \frac{1}{n}\sum_{i=1}^n y_i
\end{equation}
ein potenzieller Maximum-Likelihood-Schätzer von $\mu$. Einsetzen dieses Schätzers
...
\end{equation}
ein potenzieller Maximum-Likelihood-Schätzer von $\sigma^2$. Beide potenziellen
...
dass $\hat{\mu}^{\mbox{\tiny ML}}$ mit dem Stichprobenmittel $\bar{y}$ identisch ist, aber
...
:::{#exm-anwendungsbeispiel-parameterschaetzung}
## Anwendungsbeispiel

Zum Abschluss dieses Abschnitts wollen wir @thm-maximum-likelihood-schaetzer-des-normalverteilungsmodells
im Kontext des Anwendungsbeispiels aus @exm-anwendungsbeispiel betrachten.
...
von Schätzern in Abhängigkeit der Verteilung der ihnen zugrundeliegenden Daten.
Weil Daten in der Frequentistischen Inferenz zufällig sind, sind auch Schätzer zufällig.
Speziell werden beobachtete Datenwerte als Realisierungen von Zufallsvariablen interpretiert.
...
dieses Abschnittes und betreffen die Eigenschaften eines Schätzers für einen festen
...
\end{equation}
auch wiederholt solche Schätzer, die bei parametrischen Produktmodellen
nur Funktionen der Parameter wie den Erwartungswert, die Varianz oder die Standardabweichung
der Stichprobenvariablen schätzen. Da nach Annahme die Verteilungen der
Stichprobenvariablen $y_1,...,y_n$ identisch sind, handelt es sich dabei
...
\tau(\theta) := \mathbb{V}_\theta(y_1) \mbox{ und }
...
führen wir mit den Begriffen der *Varianz* und des *Standardfehlers* eines Schätzers
zwei Maße für die Frequentistische Variabilität von Schätzern ein. Mit dem *mittleren
quadratischen Fehler* eines Schätzers $\hat{\tau}$ als Erwartungswert der quadrierten Abweichung von
$\hat{\tau}(y)$ von $\tau(\theta)$ führen wir dann in @sec-mittlerer-quadratischer-fehler
...
in Beziehung zu setzen. Die in @sec-cramer-rao-ungleichung diskutierte
...
$y = (y_1,...,y_n)$ sei eine Stichprobe eines Frequentistischen Inferenzmodells und
$\hat{\tau}$ sei ein Schätzer für $\tau$.
...
\mathbb{E}_{\theta}(\hat{\tau} (y)) = \tau(\theta) \mbox{ für alle } \theta \in \Theta \mbox{ und alle } n \in \mathbb{N}.
\end{equation}
...
Fehler eines Schätzers von der spezifischen Realisation der Stichprobe $y$
abhängt. Der systematische Fehler dagegen ist der erwartete Fehler über
Stichprobenrealisationen und damit von einer spezifischen Realisation unabhängig.
Für den speziellen Fall eines Parameterpunktschätzers gilt nach
@def-fehler-systematischer-fehler-und-erwartungstreue, dass er erwartungstreu ist,
wenn gilt, dass
\begin{equation}
...
## Erwartungstreue von Stichprobenmittel und Stichprobenvarianz
$y := (y_1,...,y_n)$ sei die Stichprobe eines parametrischen Produktmodells.
Dann gelten
...
:::{.proof}
\noindent (1) Die Erwartungstreue des Stichprobenmittels ergibt sich mit
@thm-erwartungswert-bei-linear-affiner-kombination-von-zufallsvariablen aus
\begin{align}
...
wir zunächst fest, dass mit @thm-varianz-einer-linearkombination-von-zufallsvariablen
bei Unabhängigkeit der betrachteten Zufallsvariablen gilt, dass
\begin{equation}
...
$y_i$ mit $1 \le i \le n$. Man beachte, dass im Beweis der Erwartungstreue der
...
\end{equation}
:::
Allgemein führen nicht-lineare Transformationen von erwartungstreuen Schätzern
oft auf verzerrte Schätzer, was wir hier aber nicht weiter vertiefen wollen.

:::{#exm-simulationsbeispiel-erwartungstreue}
## Simulationsbeispiel
...
viele Realisierungen von $y_1,...,y_{12}$ als Funktion der Anzahl an
Realisierungen (Simulationen) geschätzt.
...
for(sim in 1:nsim){
    ...
    # Erwartungswert-, Varianz-, Standardabweichung-Schätzer
...
E_hat_y_bar = cumsum(y_bar)/(1:nsim) # \mathbb{E}(\bar{y}) Schaetzungen
...
Stichprobenvariablen $y_1,...,y_{12}$ sowie die wahren, aber unbekannten,
Werte des Erwartungswerts, der Varianz und der Standardabweichung der $y_1,...,y_{12}$.
Es fällt auf, dass diese Schätzungen bei geringer Realisierungsanzahl
variabler ausfallen. Ab einer Schätzung basierend auf etwa 10.000 Realisierungen
von $y_1,...,y_{12}$ entsprechen die Stichprobenmittel von $\bar{y}$ und $S^2$
gemäß ihrer Erwartungstreue ihren wahren, aber unbekannten, Werten.
Die Stichprobenstandardabweichung dagegen zeigt gemäß ihrer Verzerrtheit
auch bei weiter ansteigenden Anzahlen von der Realisierungen von $y_1,...,y_{12}$
...
# Visualisierung
library(latex2exp)
pdf("./_figures/402-erwartungstreue.pdf", width = 7, height = 5)
fig  = par(
family      = "sans",
bty         = "l",
lwd         = 1,
...
lty         = 1,
ylab        = " ",
xlab        = TeX("Anzahl an Realisierungen von $y_1,...y_{12}$"),
ylim        = c(1.3,2.2))
...
Standardabweichung bei normalverteilten Stichprobenvariablen](./_figures/402-erwartungstreue){#fig-erwartungstreue fig-align="center"}
...
In diesem Abschnitt betrachten wir seine Varianz und seine Standardabweichung.
Wir nutzen folgende Definition.

:::{#def-varianz-und-standardfehler}
## Varianz und Standardfehler
$y = (y_1,...,y_n)$ sei die Stichprobe eines Frequentistischen Inferenzmodells
...
Zufallsvariable $\hat{\tau}(y)$ definiert. Der Standardfehler eines Schätzers
$\hat{\tau}$ ist als die Standardabweichung von $\hat{\tau}(y)$ definiert. Als
...
$y = (y_1,...,y_n)$ sei die Stichprobe eines parametrischen Produktmodells.
Dann ist der \textit{Standardfehler des Stichprobenmittels} gegeben durch
\begin{equation}
...
Da die Standardabweichung $\mathbb{S}_\theta(y_1)$ unbekannt ist, ist auch der
Standardfehler $\mbox{SE}(\bar{y})$ unbekannt, kann also nur geschätzt werden.
Mit der Stichprobenstandardabweichung als verzerrtem Schätzer der Standardabweichung
$\mathbb{S}_\theta(y_1)$ ergibt sich ein ebenfalls verzerrter Schätzer für
den Standardfehler des Stichprobenmittels als
\begin{equation}
...
$y = (y_1,...,y_n)$ sei die Stichprobe eines Bernoullimodells und
...
\end{split}
\end{align}
wobei die dritte Gleichung mit der Unabhängigkeit der $y_i$ und die
vierte Gleichung mit der Varianz
\begin{equation}
\mathbb{V}_\mu(y_1) = \mathbb{V}_\mu(y_i) = \mu(1-\mu)
\end{equation}
der Stichprobenvariablen folgt.
:::
...
:::{#exm-simulationsbeispiel-standardfehler}
## Simulationsbeispiel
...
dabei offenbar vom Stichprobenumfang ab. Höhere Stichprobenumfänge resultieren
in einer geringeren Variabilität des Schätzers. Diesen Gedanken werden wir in
@sec-asymptotische-schaetzereigenschaften vertiefen.
...
# Visualisierung
library(latex2exp)
pdf(file  = file.path("./_figures/402-sem.pdf"), width = 10, height = 3.3)
fig  = par(
family      = "sans",
bty         = "l",
mfcol       = c(1,3),
...
Die Variabilität des Schätzers hängt dabei offenbar vom Stichprobenumfang $n$ ab.](./_figures/402-sem){#fig-sem fig-align="center"}
...
Schätzers ermöglicht eine integrierte Betrachtung der Genauigkeit (Erwartungstreue)
und Variabilität (Varianz) eines Schätzers im Sinne seiner sogenannten
*Bias-Varianz-Zerlegung*. Wir definieren den mittleren quadratischen Fehler
eines Schätzers zunächst wie folgt.

:::{#def-mittlerer-quadratischer-fehler}
## Mittlerer quadratischer Fehler
$y = (y_1,...,y_n)$ sei die Stichprobe eines parametrischen
...
Abweichung von $\hat{\tau}(y)$ von $\tau(\theta)$. Man beachte, dass in Abgrenzung
dazu die Varianz von $\hat{\tau}$ die erwartete quadrierte Abweichung von
$\hat{\tau}$ von $\mathbb{E}_\theta(\hat{\tau}(y))$ ist. Dabei kann, wie in
@sec-erwartungstreue gesehen $\mathbb{E}_\theta(\hat{\tau}(y))$ mit $\tau(\theta)$
...
$\hat{\tau} := \hat{\tau}(y)$ und $\bar{\tau}_n := \mathbb{E}_\theta(\hat{\tau}(y))$.
Dann gilt:
\begin{align}
\begin{split}
...
welche wiederum auf dem Begriff der *Scorefunktion* eines Frequentistischen Inferenzmodells
...
selbst identifizierbar ist, dass also WMFen oder WDFen mit unterschiedlichen Parameterwerten
...
wollen nur Modelle mit eindimensionalem Parameterräumen $\Theta \subseteq \mathbb{R}$
...
abhängen, sind sie vor dem Hintergrund eines Frequentistischen Inferenzmodells
...
Es sei $y = (y_1,...,y_n)$ die Stichprobe eines Bernoullimodells mit
...
$y = (y_1,...,y_n)$ sei die Stichprobe eines Normalverteilungsmodells
...
Maximum-Likelihood-Schätzers $\hat{\mu}^{\mbox{\tiny ML}}_n$. Die erwartete
...
bei bekanntem Erwartungswertparameter $\mu$ durch
\begin{equation}
...
\end{align}
Die beobachtete Fisher-Information ist die Fisher-Information an der Stelle des
Maximum-Likelihood-Schätzers $\hat{\sigma}^{2^{\mbox{\tiny ML}}}$,
\begin{align}
\begin{split}
...
Gegeben sei ein Frequentistisches Inferenzmodell mit eindimensionalem Parameter
...
großen Stichprobenumfängen beschäftigt. Dabei werden Methoden der Asymptotischen
Statistik zum einen benutzt um, wie hier, qualitative Schätzereigenschaften zu
studieren und andererseits um Schätzereigenschaften bei großen Stichprobenumfängen
approximieren zu können. Da Stichprobenumfänge heutzutage durchaus groß sein können,
sind die Methoden der Asymptotischen Statistik also für die Anwendung gut motiviert
und dort vielseitig einsetzbar.
...
Schätzer $\hat{\tau}_n$. In @sec-asymptotische-erwartungstreue betrachten wir die
*Asymptotische Erwartungstreue* eines Schätzers. Dabei heißt ein Schätzer
...
$n \to \infty$ die Wahrscheinlichkeit dafür, dass $\hat{\tau}_n(y)$ vom wahren,
...
In @sec-asymptotische-normalitaet führen wir mit dem Begriff der *Asymptotischen Normalverteilung*
eine entsprechende Formalisierung ein. Ein Schätzer $\hat{\tau}_n$ für $\tau$
heißt dann *asymptotisch normalverteilt*, wenn für große Stichprobenumfänge
$n \to \infty$, die Verteilung von $\hat{\tau}_n$ durch eine Normalverteilung gegeben ist.
In @sec-asymptotische-effizienz schließlich betrachten wir mit der *Asymptotischen Effizienz*
ein Optimalitätskriterium für Schätzer bei gegen unendlich strebenden Stichprobenumfängen.
Danach heißt ein Schätzer $\hat{\tau}_n$ für $\tau$ *asymptotisch effizient*, wenn
für große Stichprobenumfänge $n \to \infty$ die Verteilung von $\hat{\tau}_n$
durch eine Normalverteilung mit Erwartungswertparameter $\tau(\theta)$ und
Varianzparameter gleich der Cramér-Rao-Schranke gegeben ist.
...
\begin{equation}
\lim_{n\to\infty} \mathbb{E}_\theta(\hat{\tau}_n(y)) = \tau(\theta) \mbox{ für alle } \theta \in \Theta.
\end{equation}
:::
...
$y = (y_1,...,y_n)$ sei die Stichprobe eines Normalverteilungsmodells mit
Varianzparameter $\sigma^2$. Dann ist der Maximum-Likelihood-Schätzer von $\sigma^2$,
\begin{equation}
...
:::{.proof}
Mit der Erwartungstreue der Stichprobenvarianz ergibt sich zunächst
(vgl. @thm-erwartungstreue-von-stichprobenmittel-und-stichprobenvarianz)
\begin{equation}
...
:::{#exm-simulationsbeispiel-asymptotische-erwartungstreue}
...
ns        = 1e3                                             # Anzahl Simulationen pro Stichprobenumfang
...
E_sigsqr_ml = colMeans(sigsqr_ml)	                        # Schätzererwartungswertschätzung
```
\normalsize
...
library(GMCM)
pdf(file  = file.path("./_figures/402-sigsqr-ml.pdf"), width = 6, height = 4)
fig  = par(
family      = "sans",
bty         = "l",
lwd         = 1,
...
bei größeren Stichprobenumfängen ist der Schätzer in etwa erwartungstreu.](./_figures/402-sigsqr-ml){#fig-sigsqr-ml fig-align="center" width=80%}
...
Die Intuition zu @def-konsistenz entspricht dem Gesetz der Großen Zahlen.
Für Stichprobenumfänge mit $n \to \infty$ wird die Wahrscheinlichkeit, dass
$\hat{\tau}_n(y)$ beliebig nah bei $\tau(\theta)$ liegt bzw. die Wahrscheinlichkeit,
dass $\hat{\tau}_n(y)$ weit von $\tau(\theta)$ abweicht, klein. Allerdings müssen
...
für den Erwartungswertparameter eines Normalverteilungsmodells.

:::{#exm-simulationsbeispiel-konsistenz}

Analog zu @sec-gesetze-der-grossen-zahlen demonstrieren wir die Bedeutung der
Konsistenz mithilfe der Simulation für $\mu = 1$ und $\sigma^2 = 2$ anhand
folgenden **R** Codes.
...
# Schaetzung von \mathbb{P}(|\hat{\tau}_n(y)-\tau(\theta)| \ge \epsilon)
P_hat       = apply(E, c(1,2), mean)
```
\normalsize

Wir visualisieren die Schätzung der Wahrscheinlichkeit $\mathbb{P}(|\hat{\tau}_n(y)-\tau(\theta)| \ge \epsilon)$
...
Stichprobenumfänge für eine geringe Wahrscheinlichkeit der Abweichung des Schätzers
vom wahren, aber unbekannten, Parameterwert, bei kleineren Werten von $\epsilon$
sind dazu größere Stichprobenumfänge nötig.
:::

```{r, eval = FALSE, echo = FALSE}
# Visualisierung
pdf(file = file.path("./_figures/402-konsistenz.pdf"), width = 6, height = 4.5)
fig  = par(
family      = "sans",
bty         = "l",
lwd         = 1,
...
dev.off()
```
![Simulation der Konsistenz des Stichprobenmittels als Schätzer für den Erwartungswertparameter des Normalverteilungsmodells.](./_figures/402-konsistenz){#fig-konsistenz fig-align="center" width=60%}
...
\begin{equation}
\mathbb{P}_\theta\left(|\hat{\tau}_n(y) - \tau(\theta)| \ge \epsilon \right) \le
\frac{\mathbb{E}_\theta\left((\hat{\tau}_n(y) -  \tau(\theta))^2\right)}{\epsilon^2}
\end{equation}
Grenzwertbildung ergibt dann
\begin{equation}
\lim_{n\to \infty}\mathbb{P}_\theta\left(|\hat{\tau}_n(y) - \tau(\theta)| \ge \epsilon \right) \le
\frac{1}{\epsilon^2}\lim_{n\to\infty}\mathbb{E}_\theta\left((\hat{\tau}_n(y) - \tau(\theta))^2\right).
\end{equation}
Wenn also $\lim_{n\to\infty}\mathbb{E}_\theta\left((\hat{\tau}_n(y) - \tau(\theta))^2\right) = 0$
gilt, dann gilt mit $\mathbb{P}_\theta(|\hat{\tau}_n(y) - \tau(\theta)| \ge \epsilon)\ge 0$, dass
\begin{equation}
\lim_{n\to \infty}\mathbb{P}_\theta\left(|\hat{\tau}_n(y) - \tau(\theta)| \ge \epsilon \right) = 0.
\end{equation}
...
Es sei $y$ die Stichprobe eines Normalverteilungsmodells. Dann ist $\bar{y}_n$
ein konsistenter Schätzer von $\mathbb{E}(y_1)$.
:::
...
Als Beispiel für einen asymptotisch normalverteilten Schätzer betrachten wir in
@sec-asymptotische-effizienz den Maximum-Likelihood-Schätzer des Bernoullimodellparameters.
...
:::{#exm-simulationsbeispiel-asymptotische-effizienz}

Als Beispiel für einen asymptotisch effizienten Schätzer betrachten wir den
Maximum-Likelihood-Schätzer des Bernoullimodellparameters. Folgender **R** Code simuliert die Frequentistische Verteilung dieses Schätzers und bestimmt die
...
mu_hat_y = seq(0,1,len = mu_hat_r)   		 		                            # Maximum-Likelihood-Schätzerraum
...
\normalsize
```{r, echo = FALSE, eval = FALSE}
pdf(file  = file.path("./_figures/402-effizienz.pdf"), width = 12, height = 4)
fig  = par(
family      = "sans",
bty         = "l",
mfcol       = c(1,3),
...
formulierten Normalverteilung](./_figures/402-effizienz){#fig-effizienz fig-align="center" width=90%}

## Eigenschaften von Maximum-Likelihood-Schätzern {#sec-eigenschaften-von-maximum-likelihood-schaetzern}

Das Maximum-Likelihood-Prinzip zur Gewinnung von Schätzern für Parameter Frequentistischer
Inferenzmodelle ist durch folgendes Theorem zu den Eigenschaften von
...
$y$ sei die Stichprobe eines parametrischen Produktmodells und
...
Für einen Beweis von @thm-eigenschaften-von-maximum-likelihood-schätzern
verweisen wir auf @held2014, Abschnitt 3.4. Nutzt man also das Maximum-Likelihood-Prinzip
zur Gewinnung eines Schätzers, so erfüllt der gewonnene Schätzer garantiert die in
@thm-eigenschaften-von-maximum-likelihood-schätzern zusammengefassten Schätzergütekriterien.
...
1. Geben Sie die Definition der Begriffe der Likelihood-Funktion und der Log-Likelihood-Funktion wieder.
1. Geben Sie die Definition des Begriffs des Maximum-Likelihood-Schätzers wieder.
...
1. Geben Sie die Definition der Begriffe der Varianz und des Standardfehlers eines Schätzers wieder.
1. Geben Sie die Definition des Begriffs der Asymptotischen Erwartungstreue eines Schätzers wieder.
1. Erläutern Sie den Begriff der asymptotischen Erwartungstreue eines Schätzers.
1. Geben Sie die Definition des Begriffs der Konsistenz eines Schätzers wieder.
1. Erläutern Sie den Begriff der Konsistenz eines Schätzers.
1. Geben Sie die ersten drei Aussagen des Theorems zu den Eigenschaften von Maximum-Likelihood-Schätzern wieder.

## Lösungen {.unnumbered .unlisted}

\footnotesize
1. Siehe @def-parameterpunktschätzer.
1. Ein Parameterpunktschätzer gibt basierend auf einer Stichprobe einen Tipp
für den entsprechenden wahren, aber unbekannten, Parameter an.
1. Siehe @def-likelihood-funktion-und-log-likelihood-funktion.
1. Siehe @def-maximum-likelihood-schaetzer.
1. Um einen Maximum-Likelihood-Schätzer zu gewinnen, formuliert man zunächst die
Log-Likelihood-Funktion und bestimmt dann die Nullstellen ihrer Ableitung als
potenzielle Maximumstellen. Dazu nutzt man in klassischen Beispielen meist die
analytische, in der Anwendung auf komplexere Modelle dagegen meist die numerische Optimierung.
1. Siehe @thm-maximum-likelihood-schaetzer-des-bernoullimodells.
1. Siehe @thm-maximum-likelihood-schaetzer-des-normalverteilungsmodells.
1. Siehe @def-fehler-systematischer-fehler-und-erwartungstreue.
1. Ein Schätzer heißt erwartungstreu, wenn sein Erwartungswert mit dem von ihm
geschätzten wahren, aber unbekannten, Wert identisch ist.
1. Siehe @def-varianz-und-standardfehler.
1. Siehe @def-asymptotische-erwartungstreue.
1. Ein Schätzer heißt asymptotisch erwartungstreu, wenn sein Erwartungswert für
gegen unendlich gehende Stichprobenumfänge mit dem von ihm geschätzten wahren,
aber unbekannten, Wert identisch ist.
1. Siehe @def-konsistenz.
1. Ein Schätzer heißt konsistent, wenn für große Stichprobenumfänge die
Wahrscheinlichkeit dafür, dass der Schätzerwert vom wahren, aber unbekannten,
Wert abweicht, beliebig klein wird.
1. Siehe @thm-eigenschaften-von-maximum-likelihood-schätzern, Aussagen (1), (2) und (3).
